...
export async function sendRedFlagAlert({ candidate, assessment, result, userEmail, threshold }) {
  ...
  // Build reasons list. The scoring engine emits shell-tailored watch-out
  // titles (healthcare: "MAR chart documentation gap"; education: "Missed
  // safeguarding referral to DSL"; office: generic competency phrasing).
  // When high-severity watch-outs are present, surface them verbatim and
  // capped at three so the email stays scannable. When the flag fired on
  // score-below-threshold or integrity alone (no high-severity watch-outs),
  // fall back to the original reasons so the recipient still sees why.
  ...
  const reasons = []
  if (highWatchouts.length > 0) {
    highWatchouts.forEach(w => {
      const title = typeof w === 'object' ? (w.text || w.title || w.watchout || 'Unnamed concern') : String(w)
      reasons.push(title)
    })
  } else {
    if (score < threshold) {
