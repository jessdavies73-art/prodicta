...
export const DEMO_ASSESSMENTS = [
  { id: 'demo-assess-1', role_title: 'Marketing Manager', status: 'active', detected_role_type: 'operations', assessment_mode: 'standard', employment_type: 'permanent', scenario_version: 'scenario-v1.0', candidate_count: 1, created_at: '2026-04-20T10:00:00Z', client_name: 'Acme SaaS', location: 'London', team_member_id: 'demo-tm-1',
  ...
  { id: 'demo-assess-2', role_title: 'Sales Executive', status: 'active', detected_role_type: 'admin', assessment_mode: 'standard', employment_type: 'permanent', scenario_version: 'scenario-v1.0', candidate_count: 1, created_at: '2026-04-18T10:00:00Z', client_name: 'Northwind Group', location: 'Manchester', team_member_id: 'demo-tm-2',
    interruption_keying: 'assessment',
    job_breakdown: { tasks: [
  ...
  { id: 'demo-assess-3', role_title: 'Software Developer', status: 'active', detected_role_type: 'finance', assessment_mode: 'standard', employment_type: 'permanent', scenario_version: 'scenario-v1.0', candidate_count: 0, created_at: '2026-04-15T10:00:00Z', client_name: 'Bright Tech', location: 'Bristol', team_member_id: 'demo-tm-2',
    interruption_keying: 'assessment',
    job_breakdown: { tasks: [
  ...
  { id: 'demo-assess-4', role_title: 'Customer Service Team Leader', status: 'active', detected_role_type: 'customer_service', assessment_mode: 'quick', employment_type: 'permanent', candidate_count: 0, created_at: '2026-04-12T10:00:00Z', client_name: 'Acme SaaS', location: 'Leeds', team_member_id: 'demo-tm-3',
    interruption_keying: 'assessment',
    job_breakdown: { tasks: [
  ...
  { id: 'demo-assess-5', role_title: 'Assistant Management Accountant', status: 'active', detected_role_type: 'finance', assessment_mode: 'standard', employment_type: 'permanent', scenario_version: 'scenario-v1.0', candidate_count: 0, created_at: '2026-04-10T10:00:00Z', client_name: 'Northwind Group', location: 'London', team_member_id: 'demo-tm-1',
    interruption_keying: 'assessment',
    job_breakdown: { tasks: [
...
// Team members for the drill-down "By Team Member" view. Real production
// data lives on the team_members table; demos render against this list
// so the agency demo experience shows three users with different roles
// (owner, manager, consultant) and a believable activity mix.
export const DEMO_TEAM_MEMBERS = [
  {
    id: 'demo-tm-1', user_id: 'demo-user-owner',
    name: 'Sarah Mitchell', email: 'sarah@demoagency.co.uk',
    role: 'owner', status: 'active',
    joined_at: '2025-09-12T09:00:00Z',
    last_active_at: '2026-04-26T11:30:00Z',
  },
  {
    id: 'demo-tm-2', user_id: 'demo-user-manager',
    name: 'David Patel', email: 'david@demoagency.co.uk',
    role: 'manager', status: 'active',
    joined_at: '2025-11-04T09:00:00Z',
    last_active_at: '2026-04-25T16:45:00Z',
  },
  {
    id: 'demo-tm-3', user_id: 'demo-user-consultant',
    name: 'Emma Walsh', email: 'emma@demoagency.co.uk',
    role: 'consultant', status: 'active',
    joined_at: '2026-01-20T09:00:00Z',
    last_active_at: '2026-04-26T09:15:00Z',
  },
]

// Clients (the companies an agency hires for). Drives the "By Client"
// drill-down. Each client maps to a set of demo assessments via the
// client_name field on DEMO_ASSESSMENTS and DEMO_CANDIDATES.
export const DEMO_CLIENTS = [
  {
    id: 'demo-client-1', name: 'Acme SaaS',
    sector: 'B2B SaaS', size: 'mid_market',
    primary_contact: 'Helen Carter, Procurement Director',
    last_activity_at: '2026-04-26T11:30:00Z',
  },
  {
    id: 'demo-client-2', name: 'Northwind Group',
    sector: 'Manufacturing', size: 'enterprise',
    primary_contact: 'Tom Webb, Head of Talent',
    last_activity_at: '2026-04-25T16:45:00Z',
  },
  {
    id: 'demo-client-3', name: 'Bright Tech',
    sector: 'FinTech', size: 'scaleup',
    primary_contact: 'Priya Shah, VP Engineering',
    last_activity_at: '2026-04-22T14:20:00Z',
  },
]
...
export function getDemoCandidatesFull() {
  return DEMO_CANDIDATES.filter(c => c.status === 'completed').map(c => {
    ...
    // Per-block Workspace scoring detail. Hand-written entries live in
    // DEMO_WORKSPACE_BLOCK_SCORES (defined later in this file). Merging
    // here keeps the candidate result shape identical to what the live
    // pipeline produces, so the candidate-report drill-down renders
    // without any branching for demos.
    const blockScores = (typeof DEMO_WORKSPACE_BLOCK_SCORES !== 'undefined' && DEMO_WORKSPACE_BLOCK_SCORES[c.id]) || null
    // When a demo carries modular block scores, stamp the matching
    // audit-trail versions so the Manager Brief and Evidence Pack PDFs
    // render the new three-row modular section against demo candidates.
    // Values mirror lib/constants PD_WORKSPACE_RUBRIC_VERSION and
    // PD_WORKSPACE_BLOCK_LIBRARY_VERSION; we hardcode the strings here
    // rather than importing to keep this module pure data.
    const modularDefaults = blockScores ? {
      workspace_rubric_version: 'workspace-rubric-v1.0',
      workspace_block_library_version: 'office-block-library-v1.0',
    } : {}
    return {
      ...c,
      results: [{
        ...
        ...auditDefaults,
        ...full,
        ...(blockScores ? { workspace_block_scores: blockScores } : {}),
        ...modularDefaults,
      }],
    }
  })
...
// ── Per-block Workspace scoring detail for the modular Office shell ───────
// Hand-written demo workspace_block_scores keyed by candidate id. Each
// entry mirrors the shape produced by lib/workspace-block-scoring.js so
// the per-block drill-down on the candidate report has data to render
// for the seven completed demo candidates. Scores per block sit close
// to the candidate's overall band, with believable variance: their
// strongest block sits at the top of their range, their weakest at the
// bottom. Block sets follow the canonical role mapping for each role.
export const DEMO_WORKSPACE_BLOCK_SCORES = {
  // Sophie Chen, Marketing Manager (92): top of the range, strong on
  // every block, dominant on stakeholder-conflict and decision-queue.
  'demo-c1': [
    {
      block_id: 'spreadsheet-data', score: 90,
      strengths: [
        'Caught both planted anomalies in the channel attribution table within 90 seconds, naming the row and the metric in her note.',
        'Headline insight in the summary led with commercial impact, not table description.',
        'Recommended action was anchored to a specific row and a named owner.',
      ],
      watch_outs: [
        'Slight tendency to highlight adjacent cells alongside the true anomaly; trim noise so the flagged cells stay punchy.',
      ],
      narrative: 'Evidence suggests Sophie reads tables for headline first, detail second. Her cell notes interpret rather than describe, and the summary closes with a specific next step grounded in the numbers she flagged.',
      signals: [
        { type: 'anomaly_catch', evidence: 'Identified attribution concentration on row 4 and the negative cost outlier on row 11.', weight: 'high' },
        { type: 'commercial_framing', evidence: 'Summary names the £140k spend at risk, not just the variance.', weight: 'high' },
        { type: 'note_interpretation', evidence: 'Notes name the cause hypothesis, not just the value.', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 93,
      strengths: [
        'Each decision had a clear chosen option and a rationale that named the trade-off explicitly.',
        'Decision speed scaled with weight: snap on small, deliberate on board-level.',
        'Consistently named the affected stakeholder by role in her rationales.',
      ],
      watch_outs: [],
      narrative: 'Indicators show calibrated decision making across the queue. Sophie commits early on lightweight calls and slows appropriately on heavier ones, with rationales that engage the named constraint rather than hedge around it.',
      signals: [
        { type: 'trade_off_articulation', evidence: 'Rationale on the budget freeze decision named the campaign she would cut and why.', weight: 'high' },
        { type: 'speed_calibration', evidence: '38s on a this_week call, 142s on a board-level call.', weight: 'medium' },
        { type: 'constraint_engagement', evidence: 'Three rationales reference the stated constraint by name.', weight: 'high' },
      ],
    },
    {
      block_id: 'document-writing', score: 91,
      strengths: [
        'Hit every must-include item in the brief and stayed inside the 600-word limit.',
        'Opening reframes the question for the CEO before the recommendation.',
        'Closing names the next decision and the deadline.',
      ],
      watch_outs: [
        'Two passive constructions in the body could be tightened for a CEO audience.',
      ],
      narrative: 'The board update reads as the work of a candidate who has written for senior audiences before. Structure is tight, language is plain, and the recommendation is clearly the candidate\'s call.',
      signals: [
        { type: 'audience_fit', evidence: 'Register matches CEO-level brevity; no marketing jargon.', weight: 'high' },
        { type: 'must_include_coverage', evidence: 'All five must-include points addressed in order.', weight: 'high' },
        { type: 'structure', evidence: 'Clear context, body, close pattern.', weight: 'medium' },
      ],
    },
    {
      block_id: 'stakeholder-conflict', score: 94,
      strengths: [
        'Each stakeholder response acknowledged the named want before pivoting.',
        'Final decision was clear, defensible, and explicitly named who would be unhappy and why.',
        'Public message and private strategy were genuinely different layers.',
      ],
      watch_outs: [],
      narrative: 'Strong evidence of political judgement. Sophie weighted the high-power stakeholder concerns appropriately and held a layer between the public line and the private playbook.',
      signals: [
        { type: 'power_weighting', evidence: 'Final decision favoured the FD\'s constraint over the BD lead\'s preference.', weight: 'high' },
        { type: 'discretion', evidence: 'Public message was even-handed; private strategy named who she\'d call first.', weight: 'high' },
        { type: 'acknowledgement', evidence: 'All four stakeholder responses opened with the stakeholder\'s stated want.', weight: 'medium' },
      ],
    },
    {
      block_id: 'crisis-simulation', score: 92,
      strengths: [
        'Stage 1 named the holding line and who she would call before committing to anything.',
        'Stage 2 explicitly referenced the new constraint and updated the plan.',
        'Reflection named a specific moment she would handle differently with a reason.',
      ],
      watch_outs: [
        'Stage 3 response slightly long for the five-minute window; trim for executive cadence.',
      ],
      narrative: 'Composure across all three stages with adaptive thinking on stage 2. The reflection shows real self-awareness about a specific commitment moment, not generic regret.',
      signals: [
        { type: 'composure', evidence: 'Stage 1 holding line was structured and named the right escalation contact.', weight: 'high' },
        { type: 'adaptation', evidence: 'Stage 2 response opened by acknowledging the £45k late-make-good fee.', weight: 'high' },
        { type: 'reflection_specificity', evidence: 'Named the moment she would not have committed to a refund without finance.', weight: 'medium' },
      ],
    },
  ],

  // Marcus Williams, Marketing Manager (74): credible mid-pack, strong
  // on the structured blocks, softer on dynamic ones where compromise
  // judgement is tested.
  'demo-c2': [
    {
      block_id: 'spreadsheet-data', score: 70,
      strengths: [
        'Caught the suspicious variance on the paid social row and noted the cost-per-lead spike.',
        'Summary opened with the headline insight rather than narrating the table.',
      ],
      watch_outs: [
        'Missed the data quality issue on row 9 (negative attribution share); summary did not flag it.',
        'Recommended action was hedged ("worth investigating") rather than naming a next step.',
      ],
      narrative: 'Evidence suggests Marcus reads tables for the obvious anomaly but stops scanning once one is found. The recommended action could be sharper for someone in a Marketing Manager seat.',
      signals: [
        ...
        { type: 'commercial_framing', evidence: 'Summary referenced the £18k overspend specifically.', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 78,
      strengths: [
        'Defensible choice on the campaign budget call with a named trade-off.',
        'Speed proportional to weight on the heavier decisions.',
      ],
      watch_outs: [
        'Rationale on the agency switch decision read as preference ("I think this is fairer") without engaging the cost constraint.',
      ],
      narrative: 'Indicators show solid baseline decision making with a soft spot when the constraint is commercial rather than relational. Marcus engages people-side decisions more readily than money-side ones.',
      signals: [
        { type: 'trade_off_articulation', evidence: 'Two of three rationales name the trade-off clearly.', weight: 'medium' },
        { type: 'constraint_engagement', evidence: 'Cost constraint on agency decision was not addressed.', weight: 'medium' },
    ...
    },
    {
      ...
      strengths: [
        'Hit four of five must-include points and stayed inside word limit.',
        'Tone is broadly right for the agency brief audience.',
      ],
      watch_outs: [
        'Missed the must-include reference to the Q3 attribution change.',
        'Closing did not name a next decision or deadline.',
      ],
      narrative: 'The brief does the job but reads as a draft rather than a finished piece. The missing must-include and the soft close both pull the document down a band.',
      signals: [
        { type: 'must_include_coverage', evidence: 'Four of five points addressed.', weight: 'medium' },
        { type: 'audience_fit', evidence: 'Register matches Bright Agency tone.', weight: 'medium' },
        { type: 'structural_close', evidence: 'No named next step in the closing paragraph.', weight: 'medium' },
      ],
    },
    {
      block_id: 'stakeholder-conflict', score: 72,
      strengths: [
        'All four stakeholder responses acknowledged the named want before answering.',
        'Public message stayed even-handed and named no one.',
      ],
      watch_outs: [
        'Final decision read as a compromise designed to avoid conflict; did not name who would be unhappy and why.',
        'Private strategy was nearly identical to the public message; the layered approach was not present.',
      ],
      narrative: 'Patterns suggest Marcus prefers consensus to commitment. The acknowledgement layer is strong, but the final call hedges and the private strategy does not differ enough from the public message to read as discretion.',
      signals: [
        { type: 'compromise_preference', evidence: 'Final decision split the budget rather than committing to the high-power stakeholder\'s line.', weight: 'high' },
        { type: 'acknowledgement', evidence: 'All four responses named the stakeholder\'s stated want.', weight: 'medium' },
        { type: 'discretion_collapse', evidence: 'Private strategy added one sentence beyond the public message.', weight: 'medium' },
      ],
    },
    {
      block_id: 'crisis-simulation', score: 74,
      strengths: [
        'Stage 1 named the agency contact and the holding line cleanly.',
        'Final reflection named the typo cost and what he would do about sign-off.',
      ],
      watch_outs: [
        'Stage 2 stuck to the original plan despite the £45k late-make-good fee being introduced as new information.',
        'Stage 3 response hedged on whether the CEO should be told before the airing.',
      ],
      narrative: 'Indicators show competent stage-1 composure but limited adaptation when constraints shift mid-event. Stage 3 hedge on the CEO decision is the watch-out for a Marketing Manager who will need to make these calls solo.',
      signals: [
        { type: 'stage1_composure', evidence: 'Holding line in stage 1 was structured.', weight: 'medium' },
        { type: 'adaptation_gap', evidence: 'Stage 2 plan unchanged despite new £45k cost.', weight: 'high' },
        { type: 'reflection_quality', evidence: 'Reflection named the sign-off process change specifically.', weight: 'medium' },
      ],
    },
  ],

  // Priya Patel, Sales Executive (61): warm and persistent, but
  // commercial discipline gaps show up clearly in the dynamic blocks.
  'demo-c3': [
    {
      block_id: 'inbox', score: 65,
      strengths: [
        'Replies to peer-relationship emails were warm and well-calibrated.',
        'Deferred the FYI emails appropriately.',
      ],
      watch_outs: [
        'Opened in inbox order rather than priority order; the senior client escalation was the fourth email opened, not the first.',
        'Reply to the senior partner used the same casual tone as her reply to a peer.',
      ],
      narrative: 'Evidence suggests Priya defaults to inbox order and a single warm register. Tone calibration to senior and external senders is the priority development area.',
      signals: [
        ...
        { type: 'completeness', evidence: 'Replied to all must-respond emails.', weight: 'medium' },
      ],
    },
    {
      block_id: 'conversation-simulation', score: 58,
      strengths: [
        'Discovery-first instinct in the opening turn was strong.',
        'Held the relationship even when the counterpart pushed.',
      ],
      watch_outs: [
        'Conceded on price in turn 4 when the counterpart had not actually asked for a discount yet.',
        'Reflection named "I could have been clearer" rather than a specific moment.',
      ],
      narrative: 'Patterns suggest Priya prioritises rapport over commercial discipline. Conceding before the ask is the watch-out; the candidate may benefit from structured objection-handling practice.',
      signals: [
        { type: 'pre_emptive_concession', evidence: 'Offered a 10 percent discount in turn 4 unprompted.', weight: 'high' },
        { type: 'rapport_strength', evidence: 'Counterpart\'s tone softened by turn 6.', weight: 'medium' },
        { type: 'reflection_generic', evidence: 'Reflection used generic language not specific to a turn.', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 62,
      strengths: [
        'Picked the safer option on each call, with consistent rationale.',
        'Considered the named affects line on three of five decisions.',
      ],
      watch_outs: [
        'Slow on lightweight calls (over 90 seconds on a this_week decision); fast on heavyweight (under 60 seconds on a minutes-pressure call).',
        'Rationales referenced "what feels right" rather than the named constraint twice.',
      ],
      narrative: 'Indicators show preference-driven decision making rather than constraint-driven. Speed-to-weight calibration is inverted on two of five calls.',
      signals: [
        ...
        { type: 'safe_option_bias', evidence: 'Chose the conservative option on four of five decisions.', weight: 'medium' },
      ],
    },
    {
      block_id: 'stakeholder-conflict', score: 60,
      strengths: [
        'All stakeholder responses showed warmth and acknowledged the relationship.',
      ],
      watch_outs: [
        'Final decision did not commit to a clear call; named both options as "valid".',
        'Power weighting was equal across stakeholders despite low/high power split.',
      ],
      narrative: 'Patterns suggest Priya struggles to commit to a call when the stakeholders disagree. The acknowledgement layer is present; the decision layer is not.',
      signals: [
        { type: 'no_commit', evidence: 'Final decision hedged between options.', weight: 'high' },
        { type: 'equal_weighting', evidence: 'High-power and low-power stakeholders treated identically.', weight: 'medium' },
        { type: 'acknowledgement_strength', evidence: 'All four responses opened with the stated want.', weight: 'low' },
      ],
    },
  ],

  // James O'Brien, Sales Executive (35): low band; concrete missteps
  // across blocks. Watch-outs are explicit.
  'demo-c4': [
    {
      block_id: 'inbox', score: 40,
      strengths: [
        'Did reply to every email in the inbox.',
      ],
      watch_outs: [
        'Opened in inbox order; the must-respond escalation was opened last.',
        'Tone was identical for senior, peer, and external senders.',
      ],
      narrative: 'Evidence suggests James worked through the inbox like a queue rather than triaging. The ordered approach is consistent but the prioritisation criteria are not visible in the timestamps.',
      signals: [
        { type: 'inbox_order_default', evidence: 'Email open timestamps follow received_at order.', weight: 'high' },
        { type: 'flat_tone', evidence: 'Same opening salutation across all six replies.', weight: 'high' },
        { type: 'completeness', evidence: 'All emails responded to.', weight: 'medium' },
      ],
    },
    {
      block_id: 'conversation-simulation', score: 32,
      strengths: [
        'Maintained professional register throughout.',
      ],
      watch_outs: [
        'Replies read as scripted; did not respond to the specific objections the counterpart raised in turns 3 and 5.',
        'Held the same line in turn 7 as in turn 1, despite the counterpart\'s position shifting.',
      ],
      narrative: 'Indicators show a candidate delivering a prepared message rather than engaging the counterpart. The lack of adaptation across the dialogue is the dominant signal.',
      signals: [
        { type: 'scripted_responses', evidence: 'Three turns repeated the same value-prop framing.', weight: 'high' },
        { type: 'objection_skip', evidence: 'Pricing objection in turn 5 was not addressed.', weight: 'high' },
        { type: 'professional_register', evidence: 'No tone slip under pressure.', weight: 'low' },
      ],
    },
    {
      block_id: 'decision-queue', score: 38,
      strengths: [],
      watch_outs: [
        'Chose options that ignored the named constraint on three of four decisions.',
        'Rationales were one-line and did not engage the affects line.',
      ],
      narrative: 'Patterns suggest constraint-blindness across the queue. James reaches for the option that feels safest in isolation rather than the one that addresses the stated constraint.',
      signals: [
        { type: 'constraint_blindness', evidence: 'Budget cap on decision 2 was not referenced in the rationale.', weight: 'high' },
        { type: 'shallow_rationale', evidence: 'Rationales averaged 12 words.', weight: 'high' },
        { type: 'speed_uniform', evidence: 'Time-to-decide flat across light and heavy calls.', weight: 'medium' },
      ],
    },
    {
      block_id: 'stakeholder-conflict', score: 30,
      strengths: [],
      watch_outs: [
        'Public message named one of the stakeholders directly, breaking discretion.',
        'Stakeholder responses dismissed the low-power stakeholder\'s want with a one-line "this is not the priority".',
      ],
      narrative: 'Evidence suggests difficulty managing political nuance. Naming a stakeholder in the public message is a discretion failure that would carry through to the role.',
      signals: [
        { type: 'discretion_break', evidence: 'Public message named the FD by title.', weight: 'high' },
        { type: 'low_power_dismissal', evidence: 'One-line dismissal of the junior stakeholder.', weight: 'high' },
        { type: 'no_layered_strategy', evidence: 'Private strategy was three sentences shorter than public.', weight: 'medium' },
      ],
    },
  ],

  // Elena Rodriguez, Software Developer (85): strong technical clarity,
  // crisp written work, slight thinness on the reflective layer.
  'demo-c5': [
    {
      block_id: 'conversation-simulation', score: 84,
      strengths: [
        'Held architecture trade-offs cleanly when the PM pushed for a faster path.',
        'Each turn responded to the specific concern in the previous turn.',
      ],
      watch_outs: [
        'Reflection named the technical lesson but not the relationship lesson.',
      ],
      narrative: 'Indicators show clear technical communication paired with active listening. The dialogue advanced rather than circled.',
      signals: [
        { type: 'technical_clarity', evidence: 'Named the coupling risk in plain language for a non-technical PM.', weight: 'high' },
        { type: 'active_listening', evidence: 'Each candidate turn referenced the PM\'s previous question.', weight: 'high' },
        { type: 'reflection_thin', evidence: 'Reflection focused on the bug, not on the working relationship.', weight: 'medium' },
      ],
    },
    {
      block_id: 'reading-summarising', score: 86,
      strengths: [
        'Caught the hidden coupling between the auth module and the billing module that the doc only hinted at.',
        'Question probed the doc\'s assumption that the cache invalidation was idempotent.',
        'Reading time of 4m 12s on a 1100-word doc is in the considered band.',
      ],
      watch_outs: [],
      narrative: 'Strong reading comprehension with a critical question that would force the original author to defend a load-bearing assumption.',
      signals: [
        { type: 'hidden_signal_catch', evidence: 'Identified coupling not stated explicitly in the doc.', weight: 'high' },
        { type: 'question_quality', evidence: 'Probed an assumption rather than asking for a fact.', weight: 'high' },
        { type: 'reading_pace', evidence: '262 wpm is within the considered band.', weight: 'low' },
      ],
    },
    {
      block_id: 'document-writing', score: 88,
      strengths: [
        'Post-incident note is structured, blameless, and names the contributing factors.',
        'Action items each have a named owner and a target date.',
        'Hit must-include points cleanly within the 400-word limit.',
      ],
      watch_outs: [],
      narrative: 'The post-incident note reads at the level of a senior engineer. Blameless framing without losing accountability is the standout.',
      signals: [
        { type: 'blameless_framing', evidence: 'Named the contributing factors without naming individuals.', weight: 'high' },
        { type: 'action_ownership', evidence: 'Three action items each have an owner and date.', weight: 'high' },
        { type: 'word_discipline', evidence: '388 words for a 400-word target.', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 85,
      strengths: [
        'Hotfix vs proper-fix call defended with explicit reference to the deploy window.',
        'Rationales engaged each named constraint.',
      ],
      watch_outs: [
        'Slightly fast on the heaviest call (under 60 seconds on a board-visible decision).',
      ],
      narrative: 'Patterns suggest decisive technical judgement with a small calibration gap on the heavier political calls.',
      signals: [
        { type: 'constraint_engagement', evidence: 'Deploy-window mentioned in rationale.', weight: 'high' },
        { type: 'speed_fast_on_heavy', evidence: 'Heavyweight call decided in 53 seconds.', weight: 'medium' },
        { type: 'option_fit', evidence: 'Chose the proper fix with a holdover patch.', weight: 'medium' },
      ],
    },
    {
      block_id: 'crisis-simulation', score: 82,
      strengths: [
        'Stage 1 named the on-call rotation and the holding action cleanly.',
        'Stage 2 referenced the new error-rate spike and updated the plan.',
      ],
      watch_outs: [
        'Reflection named the technical fix but not the customer-comms learning.',
      ],
      narrative: 'Indicators show solid composure across the crisis with a thin reflective layer. The technical handling is strong; the broader-context reflection is the development edge.',
      signals: [
        { type: 'composure', evidence: 'Stage 1 holding action was specific and measured.', weight: 'high' },
        { type: 'adaptation', evidence: 'Stage 2 referenced the error-rate change.', weight: 'medium' },
        { type: 'reflection_narrow', evidence: 'Reflection covered the engineering side, not the comms side.', weight: 'medium' },
      ],
    },
  ],

  // Ryan Thompson, Customer Service Team Leader (71): solid people-side,
  // some hedging on cross-team conflict.
  'demo-c10': [
    {
      block_id: 'conversation-simulation', score: 74,
      strengths: [
        'Empathetic with the frustrated customer in turns 1 and 2 before pivoting to the resolution.',
        'Held the line on the policy when the customer pushed.',
      ],
      watch_outs: [
        'Conceded on a goodwill payment in turn 5 without naming the policy precedent.',
      ],
      narrative: 'Evidence suggests Ryan reads customer emotion accurately and adjusts pace. The policy precedent layer is the development area for a Team Leader role.',
      signals: [
        { type: 'empathetic_pacing', evidence: 'Slowed the dialogue in turns 1-2.', weight: 'high' },
        { type: 'policy_precedent_gap', evidence: 'Goodwill payment offered without precedent reference.', weight: 'medium' },
        { type: 'professional_register', evidence: 'Tone stayed measured under pressure.', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 70,
      strengths: [
        'Refund authority calls were defensible and within the named limit.',
        'Rationales engaged the named constraint on three of four.',
      ],
      watch_outs: [
        'One decision referenced "we always do it this way" rather than the constraint.',
      ],
      narrative: 'Indicators show pattern-following decision making appropriate for a Team Leader, with a slight gap when the precedent does not apply.',
      signals: [
        { type: 'precedent_following', evidence: 'Three rationales referenced policy precedent.', weight: 'medium' },
        { type: 'authority_discipline', evidence: 'Refund amounts stayed within the £200 limit.', weight: 'high' },
        { type: 'precedent_overreach', evidence: 'One rationale relied on "we always" rather than the constraint.', weight: 'medium' },
      ],
    },
    {
      block_id: 'stakeholder-conflict', score: 68,
      strengths: [
        'Acknowledged both the agent\'s position and the ops constraint.',
      ],
      watch_outs: [
        'Final decision read as a compromise; did not name who would be unhappy.',
        'Public message and private strategy were close to identical.',
      ],
      narrative: 'Patterns suggest Ryan prefers to keep the team aligned rather than make a clear call when the team is split. This is recoverable with structured precedent setting.',
      signals: [
        { type: 'compromise_preference', evidence: 'Final decision split the difference between agent and ops.', weight: 'high' },
        { type: 'acknowledgement', evidence: 'Both stakeholder responses opened with the stated want.', weight: 'medium' },
        { type: 'discretion_collapse', evidence: 'Private strategy added one sentence beyond public.', weight: 'medium' },
      ],
    },
    {
      block_id: 'crisis-simulation', score: 72,
      strengths: [
        'Stage 1 named the comms director and the holding line cleanly.',
        'Stage 2 acknowledged the social-media spread and updated the plan.',
      ],
      watch_outs: [
        'Stage 3 response committed to a refund total before checking with finance.',
      ],
      narrative: 'Indicators show strong composure on the customer-facing side with a commitment-discipline gap when senior pressure compounds.',
      signals: [
        { type: 'composure', evidence: 'Stage 1 escalation contact was correctly named.', weight: 'high' },
        { type: 'commitment_discipline', evidence: 'Stage 3 named a refund figure not yet authorised.', weight: 'high' },
        { type: 'reflection_specific', evidence: 'Reflection named the financial commitment as the issue.', weight: 'medium' },
      ],
    },
    {
      block_id: 'reading-summarising', score: 71,
      strengths: [
        'Caught the policy update\'s key change to the escalation threshold.',
        'Recommendation named the team brief and the turnaround.',
      ],
      watch_outs: [
        'Question asked for a fact already in the document.',
      ],
      narrative: 'Evidence suggests adequate comprehension with a thin critical layer. The summary lifts the right headline; the question does not probe.',
      signals: [
        { type: 'headline_catch', evidence: 'Summary lifted the threshold change accurately.', weight: 'high' },
        { type: 'question_shallow', evidence: 'Question asked for a fact stated on page 1.', weight: 'medium' },
        { type: 'action_grounding', evidence: 'Recommendation named the team brief.', weight: 'medium' },
      ],
    },
  ],

  // Alex Turner, Customer Service Advisor (71): warm and well-paced,
  // delegation underdeveloped.
  'demo-c11': [
    {
      block_id: 'inbox', score: 73,
      strengths: [
        'Opened the VIP escalation first; tone matched relationship across all replies.',
        'Deferred the FYI emails appropriately.',
      ],
      watch_outs: [
        'Spent over four minutes on a routine reply that did not need that depth.',
      ],
      narrative: 'Indicators show strong calibration to sender hierarchy, with a tendency to over-invest on routine items. Triage is right; depth allocation is the development area.',
      signals: [
        { type: 'priority_order', evidence: 'VIP email opened first; FYI emails deferred.', weight: 'high' },
        { type: 'tone_calibration', evidence: 'Senior reply was measured; peer reply was direct.', weight: 'high' },
        { type: 'depth_overinvestment', evidence: '247 seconds on a routine FAQ reply.', weight: 'medium' },
      ],
    },
    {
      block_id: 'conversation-simulation', score: 72,
      strengths: [
        'Warm opening and active listening across the dialogue.',
        'Held the policy line when the customer escalated.',
      ],
      watch_outs: [
        'Reflection used generic phrasing rather than naming a specific moment.',
      ],
      narrative: 'Evidence suggests competent customer-facing dialogue with a thin reflective layer. The conversation works; the post-conversation insight does not.',
      signals: [
        { type: 'warmth', evidence: 'Tone in turns 1-2 lowered the customer\'s register.', weight: 'high' },
        { type: 'policy_hold', evidence: 'Held the refund cap line in turn 4.', weight: 'medium' },
        { type: 'reflection_generic', evidence: 'Reflection used "I could have been clearer" without a moment.', weight: 'medium' },
      ],
    },
    {
      block_id: 'task-prioritisation', score: 70,
      strengths: [
        'Order broadly matches urgency; today-deadline tasks at the top.',
      ],
      watch_outs: [
        'Kept all tasks for himself; nothing delegated despite two tasks being natural delegate candidates.',
        'Rationale on the deferred task read as preference, not constraint.',
      ],
      narrative: 'Patterns suggest a candidate who treats delegation as ownership loss. The order is right; the action mix is wrong for someone who would need to lead a small team.',
      signals: [
        { type: 'no_delegation', evidence: 'All eight tasks marked "do".', weight: 'high' },
        { type: 'order_fit', evidence: 'Today-deadline items in positions 1-3.', weight: 'medium' },
        { type: 'preference_rationale', evidence: 'Deferred task explained as "feels less urgent".', weight: 'medium' },
      ],
    },
    {
      block_id: 'decision-queue', score: 69,
      strengths: [
        'Defensible options on three of four decisions.',
      ],
      watch_outs: [
        'Slow on lightweight calls (90 seconds on a this_week decision).',
        'Rationale on the goodwill call did not name the policy precedent.',
      ],
      narrative: 'Indicators show competent baseline decision making with calibration drift on speed and precedent.',
      signals: [
        { type: 'option_fit', evidence: 'Chose defensible options across the queue.', weight: 'high' },
        { type: 'speed_slow_on_light', evidence: '90 seconds on this_week call.', weight: 'medium' },
        { type: 'precedent_gap', evidence: 'Goodwill rationale did not reference policy.', weight: 'medium' },
      ],
    },
  ],
}
...
// DEMO_MANAGER_DNA
//
// One Manager DNA profile per assessment in the demo. Each demo
// candidate's report inherits the DNA tied to their assessment_id, so
// candidates sharing an assessment share a manager profile (matches
// production behaviour: one row per assessment_id+user_id).
//
// Realistic hardcoded data per the demo parity rule. Each profile
// matches the shape produced by the live API
// (app/api/assessment/[id]/manager-dna/route.js). The candidate report
// panel only renders when account_type='employer' AND
// alignment_dimensions is non-null, so partial profiles still degrade
// gracefully.
//
// Demo coverage:
//   demo-assess-1 — Marketing Manager hiring (CMO-level manager). Used by
//     Sophie Chen (c1), Marcus Williams (c2), Tom Fletcher (c6 pending),
//     David Thompson (c9 archived).
//   demo-assess-2 — Sales Executive hiring (Sales Director-level
//     manager). Used by Priya Patel (c3), James O'Brien (c4),
//     Aisha Johnson (c7 pending).
//   demo-assess-3 — Software Developer hiring (Engineering Manager-level
//     manager). Used by Elena Rodriguez (c5), Ryan Murphy (c8 failed).
//   demo-assess-4 — Customer Service Team Leader / Advisor hiring
//     (CS Manager-level manager). Used by Ryan Thompson (c10),
//     Alex Turner (c11).
//
// All profiles are office-shell because the existing demo candidates
// are all office roles. Healthcare and education demo candidates land
// in a future demo-content expansion outside this fix.
// ─────────────────────────────────────────────────────────────────────────
export const DEMO_MANAGER_DNA = {
  'demo-assess-1': {
    management_style: 'Structured Delegator',
    delegation_approach: 'Sets a clear brief with named outcomes and a check-in cadence, then steps back. Trusts the team to own the path; intervenes when the timeline slips, not when the approach diverges from theirs.',
    conflict_style: 'Acknowledges the tension explicitly, then drives toward a decision in the same conversation. Rarely lets a disagreement become two separate conversations behind closed doors.',
    decision_speed: 'measured',
    communication_preference: 'direct',
    accountability_style: 'guided',
    ideal_candidate_traits: [
      'Comfortable working from a brief without needing the answer pre-drawn',
      'Surfaces blockers early in writing, not at the end of the week',
      'Names a recommendation when asked, not a list of options',
    ],
    clash_risk_traits: [
      'Needs constant reassurance that the work is on track',
      'Routes minor decisions back up the chain instead of owning them',
      'Avoids the difficult conversation with the agency or the sales team',
    ],
    ...
    summary: 'A measured, direct manager who sets the frame and trusts the team to execute. Strongest with people who own outcomes and surface friction in writing; will struggle with people who escalate routine decisions.',
  },

  'demo-assess-2': {
    management_style: 'Hands-On Coach',
    delegation_approach: 'Stays close to the deal and the call. Reviews call recordings and pipeline weekly, gives in-the-moment coaching rather than waiting for a 1:1. Will jump on a customer call to model the move.',
    conflict_style: 'Direct, fast, sometimes blunt. Names the issue in the same week it surfaced and expects a fix in the next forecast call. Comfortable with high tempo.',
    decision_speed: 'fast',
    communication_preference: 'direct',
    accountability_style: 'close_oversight',
    ideal_candidate_traits: [
      'Receptive to in-the-moment feedback during a live deal',
      'Brings the forecast and the friction to the 1:1 unprompted',
      'Comfortable being challenged on activity volume in front of peers',
    ],
    clash_risk_traits: [
      'Bristles at coaching mid-deal or treats feedback as criticism',
      'Forecasts optimistically and resists tighter qualification',
      'Slow to respond to coaching adjustments week to week',
    ],
    alignment_dimensions: {
    ...
    summary: 'A high-tempo, hands-on sales leader who coaches in the moment and names friction fast. Strongest with reps who absorb feedback quickly and bring honest forecasts; will clash with reps who want autonomy or treat coaching as criticism.',
  },

  'demo-assess-3': {
    management_style: 'Strategic Architect',
    delegation_approach: 'Owns the technical direction and the staff-engineer calls; delegates feature delivery and on-call rotation cleanly to leads. Spends most 1:1 time on cross-team friction and engineer growth, not standup detail.',
    conflict_style: 'Surfaces architectural disagreements in writing first, runs them in a documented decision, expects engineers to disagree-and-commit. Slower to escalate interpersonal friction; prefers it resolved between the parties.',
    decision_speed: 'measured',
    communication_preference: 'structured',
    accountability_style: 'high_autonomy',
    ideal_candidate_traits: [
      'Writes design docs without being asked and accepts written critique',
      'Owns on-call discipline without manager intervention',
      'Disagrees on technical merit then commits cleanly to the decided direction',
    ],
    clash_risk_traits: [
      'Routes architectural questions back up rather than proposing',
      'Treats disagreement as personal and surfaces it via DM after the fact',
      'Hesitates to take an on-call decision under time pressure',
    ],
    alignment_dimensions: {
    ...
    summary: 'A high-autonomy, document-first engineering manager who runs technical decisions through writing and expects engineers to own their patch. Strongest with engineers who write before they speak and own incidents cleanly; will clash with engineers who need direction in the moment.',
  },

  'demo-assess-4': {
    management_style: 'Process-First Operator',
    delegation_approach: 'Builds the playbook first, then delegates against it. Expects new joiners to learn the macros, the QA process and the escalation matrix in week one. Reviews recorded contacts weekly against the playbook.',
    conflict_style: 'Calm, structured, evidence-led. Pulls the call recording, walks through the policy, agrees the action. Avoids letting tension run; resets expectations within the same shift.',
    decision_speed: 'measured',
    communication_preference: 'structured',
    accountability_style: 'guided',
    ideal_candidate_traits: [
      'Comfortable being measured against contact metrics and QA scores',
      'Asks clarifying questions about the playbook rather than improvising',
      'Documents the customer interaction without being chased',
    ],
    clash_risk_traits: [
      'Improvises around the playbook for "speed" and skips QA notes',
      'Resists structured feedback against contact recordings',
      'Routes routine policy questions to the team lead instead of looking up the macro',
    ],
    alignment_dimensions: {
    ...
    summary: 'A structured, process-first CS leader who runs the team off recorded contacts and a documented playbook. Strongest with advisors who absorb the playbook fast and surface gaps in writing; will clash with advisors who improvise to hit handle time.',
  },
}

// Resolve a manager DNA profile by candidate id. Looks up the
// candidate's assessment_id and returns the matching DEMO_MANAGER_DNA
// entry. Returns null when:
//   - The candidate is not in DEMO_CANDIDATES.
//   - The candidate's assessment has no DNA entry.
//   - The candidate is pending / scoring_failed (panel hides).
// The candidate report panel guards on null so this matches the live
// load path's behaviour.
export function getDemoManagerDna(candidateId) {
  const cand = DEMO_CANDIDATES.find(c => c.id === candidateId)
  if (!cand) return null
  if (cand.status !== 'completed' && cand.status !== 'archived') return null
  const assessmentId = cand.assessments?.id
  if (!assessmentId) return null
  return DEMO_MANAGER_DNA[assessmentId] || null
}
