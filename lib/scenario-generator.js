...
//
// Telesales-target sector additions (audit fix for Recruitment, Law, IT):
//   L2: recruitment-consultant-junior — junior agency desk, 1 to 3 yrs
//   L3: it-manager — IT / Infrastructure / Systems Manager (function: it_infrastructure)
//   L4: cto — exec-level Chief Technology Officer (function: software_dev, c_suite)
//   L4: head-of-engineering — VP / Director of Engineering (function: software_dev, senior_manager / director)
//   L4: recruitment-director — agency MD / Recruitment Director (function: recruitment, senior_manager / director / c_suite)
//   L4: law-partner — Partner / Equity / Managing Partner (function: legal, senior_manager / director / c_suite)
//
// Substring-collision rules (Pass 1 uses naive .includes(), so role_titles
// must avoid bare 3-letter sequences that appear inside other words):
//   - 'cto' deliberately NOT in role_titles for the cto canonical: would
//     match every "DireCTOr". Bare "CTO" titles are caught via Pass 2.
//   - 'senior partner', 'practice director', bare 'partner' deliberately
//     excluded from law-partner / recruitment-director: too broad.
//   - it-manager is positioned BEFORE operations-manager so the substring
//     'it operations manager' wins over 'operations manager'.
//   - head-of-engineering is positioned BEFORE cto so director-band Pass 2
//     fan-outs land on head-of-engineering, leaving c_suite for cto.
...
export const CANONICAL_ROLE_MAPPING = [
  ...
  {
    // Junior agency recruitment consultant (1-3 years on desk). Distinct
    // from the L3 'recruitment-consultant' canonical (mid+, manages own
    // desk). Junior work is shadowing seniors, qualifying briefs,
    // sourcing candidates, learning to handle counter-offers, BD call
    // shadowing, fee-structure understanding. Block list emphasises
    // reading-summarising (job specs, candidate CVs) over crisis.
    id: 'recruitment-consultant-junior',
    label: 'Recruitment Consultant (junior, agency desk)',
    role_titles: [
      'junior recruitment consultant', 'trainee recruitment consultant',
      'graduate recruitment consultant', 'associate recruitment consultant',
      'resourcer', 'recruitment resourcer', 'candidate consultant',
      'rookie recruiter', 'rookie consultant',
    ],
    function: 'recruitment',
    seniority_bands: ['junior'],
    level: 2,
    blocks: ['inbox', 'reading-summarising', 'conversation-simulation', 'task-prioritisation'],
  },
  ...
  {
    // IT Manager / Infrastructure Manager / Systems Manager. Distinct
    // from software-developer (product engineering). The work is ticket
    // queues, vendor SLAs, security patching, identity / M365 / AD,
    // change management, on-call against system incidents, infra cost
    // control. Positioned BEFORE operations-manager so "IT Operations
    // Manager" hits this entry first via the substring 'it operations
    // manager' rather than 'operations manager'.
    id: 'it-manager',
    label: 'IT Manager / Infrastructure Manager / Head of IT',
    role_titles: [
      'it operations manager', 'it manager', 'infrastructure manager',
      'systems manager', 'helpdesk manager', 'service desk manager',
      'network manager', 'head of it', 'head of infrastructure',
      'head of systems', 'cloud operations manager',
    ],
    function: 'it_infrastructure',
    seniority_bands: ['manager', 'senior_manager'],
    level: 3,
    blocks: ['inbox', 'decision-queue', 'stakeholder-conflict', 'crisis-simulation', 'spreadsheet-data'],
  },
  {
  ...
  {
    // Head of Engineering / VP Engineering. Org-leadership scope: hiring
    // principal engineers, balancing platform vs product, OKR cascade,
    // engineering org design. Distinct from the L3 software-developer
    // canonical (tech lead / engineering manager scope) and from the
    // CTO canonical (exec / board scope). Positioned BEFORE the cto
    // canonical so director-level engineering leaders (function=
    // software_dev, seniority=director) land here on Pass 2, leaving
    // cto for c_suite-only matches. Also positioned BEFORE the generic
    // director and head-of-department entries so 'head of engineering'
    // and 'engineering director' substrings hit this first on Pass 1.
    id: 'head-of-engineering',
    label: 'Head of Engineering / VP Engineering / Director of Engineering',
    role_titles: [
      'head of engineering', 'vp engineering', 'vp of engineering',
      'director of engineering', 'engineering director',
    ],
    function: 'software_dev',
    seniority_bands: ['senior_manager', 'director'],
    level: 4,
    blocks: ['decision-queue', 'stakeholder-conflict', 'reading-summarising', 'crisis-simulation'],
  },
  {
    // CTO / Chief Technology Officer at exec level. Distinct from the
    // generic 'director' canonical so the role gets tech-leadership
    // anchors (board reporting on tech progress, build vs buy decisions,
    // exec-level security incidents, M&A tech due diligence) rather than
    // generic functional-director scaffolding.
    //
    // role_titles deliberately excludes the bare 'cto' substring because
    // 'cto' is a substring of any title containing 'direCTOr' — that
    // would route every Marketing/HR/Sales Director to this canonical
    // by accident. Bare "CTO" titles are caught via Pass 2 (function=
    // software_dev + seniority=c_suite) instead, which only fires when
    // the detector has classified the role as software_dev c_suite.
    id: 'cto',
    label: 'CTO / Chief Technology Officer',
    role_titles: ['chief technology officer', 'chief technical officer'],
    function: 'software_dev',
    seniority_bands: ['director', 'c_suite'],
    level: 4,
    blocks: ['decision-queue', 'stakeholder-conflict', 'reading-summarising', 'crisis-simulation'],
  ...
  {
    // Recruitment Director / agency MD. Distinct from the generic
    // director canonical so the role gets agency-leadership anchors
    // (placement-fee economics, biller pipeline, lateral consultant
    // churn, P&L per desk, top-client retention vs new BD) rather than
    // generic director scaffolding.
    //
    // role_titles deliberately excludes generic 'practice director'
    // (used in many sectors) and 'managing director' (handled by the
    // managing-director canonical). For an agency MD whose detector
    // tags function=recruitment, Pass 2 (function+seniority) lands
    // here when the title is bare 'managing director' alone.
    id: 'recruitment-director',
    label: 'Recruitment Director / Agency MD / Head of Desk',
    role_titles: [
      'recruitment director', 'agency director', 'agency owner',
      'recruitment md', 'md recruitment', 'recruitment managing director',
      'head of desk', 'recruitment practice director',
    ],
    function: 'recruitment',
    seniority_bands: ['senior_manager', 'director', 'c_suite'],
    level: 4,
    blocks: ['decision-queue', 'spreadsheet-data', 'stakeholder-conflict', 'crisis-simulation'],
  },
  {
    // Law Partner. Partnership economics (origination, write-offs,
    // lateral hires, supervision of fee earners, conflicts of interest
    // at partner level, succession of key clients) rather than
    // fee-earner work.
    //
    // role_titles use 'partner, ' (with trailing comma + space) to
    // catch the common directory format "Partner, Corporate Law" and
    // explicit partner-track patterns. The bare 'senior partner'
    // substring is deliberately excluded (used in consulting / PE /
    // recruitment too); for bare "Senior Partner" titles the detector's
    // legal cues route via Pass 2 (function=legal + seniority=
    // director/c_suite) which lands here.
    id: 'law-partner',
    label: 'Partner / Equity Partner / Managing Partner (legal)',
    role_titles: [
      'partner, ', 'equity partner', 'salaried partner',
      'managing partner', 'name partner',
    ],
    function: 'legal',
    seniority_bands: ['senior_manager', 'director', 'c_suite'],
    level: 4,
  ...
  {
    id: 'director',
    label: 'Director (Sales / Marketing / HR / Operations / Recruitment / PMO / Legal / Commercial)',
    // Includes specific exec acronyms (CMO, CPO, COO) which are
    // technically c_suite but read as functional directors in most
    // orgs. CTO / Chief Technology / Chief Technical and engineering /
    // recruitment director patterns are deliberately handled by their
    // own canonicals BEFORE this entry; the substring 'cto' is too
    // broad to live here (matches any "DireCTOr").
    role_titles: [
      ...
      'cmo', 'cpo', 'coo',
      'chief marketing', 'chief product', 'chief operating',
...
const FUNCTION_SCENARIO_HINTS = {
  ...
  it_infrastructure: 'Production database flipped read-only over the weekend; or a critical patch window collides with the finance system upgrade and the vendor cannot move it.',
...
const FUNCTION_DATA_FLAVOUR = {
  ...
  it_infrastructure: 'IT operations metrics: ticket volume by priority, mean-time-to-resolve by category, change failure rate, patch compliance percentage, vendor SLA hit rate, infra spend vs budget by service',
...
//
// Phase 2 Education v1.0: all 9 blocks ship as real interactive
// components with typed payloads (class-roster.pupils[],
// lesson-plan.lesson_brief, parent-communication.parent_message,
// behaviour-incident.trigger_event + pupils_involved[],
// safeguarding-referral.full_scenario_text + indicators_present[],
// head-teacher-message.head_message, cohort-coordination.demands[],
// conversation-simulation.turns[3], crisis-simulation.stages[3]).
// Per-block scorers live in lib/workspace-blocks/education/scoring/.
//
...
// Per-block content schema fields for the education shell. The first
// 5 real blocks (class-roster, lesson-plan, parent-communication,
// behaviour-incident, safeguarding-referral) emit typed payloads that
// the live UI renders directly. The remaining 4 blocks
// (head-teacher-message, cohort-coordination, conversation-simulation,
// crisis-simulation) keep the base shape until their real components
// land in the next prompt. Pattern mirrors healthcareBlockSchemaFields.
function educationBlockSchemaFields(blockId) {
  const base =
`      "summary": "string (1 sentence describing this block's content for this candidate)",
      "setup": "string (what the candidate sees / receives in this block)",
      "expected_output": "string (what the candidate is expected to produce)",
  ...
  if (blockId === 'class-roster') {
    return base + `,
      "roster_label": "string (a short label naming the cohort the candidate is looking at, e.g. 'Year 4, Class 4B', 'Tuesday morning support group', 'SEN register, Key Stage 2', 'School-wide Year 11 cohort')",
      "roster_context": "string (1-2 sentences placing the cohort in context: the year group, the time of year, what the candidate would be doing with this list this morning)",
      "pupils": [
        {
          "pupil_id": "string (unique within block, e.g. pupil-1)",
          "anonymised_name": "string (first name only, common UK name, e.g. 'Mia', 'Kai', 'Sasha', 'Jamal'. Optionally append a class letter: 'Mia (4B)'. Never a real public-figure name.)",
          "year_group_or_class": "string (e.g. 'Year 4', 'Year 11, Set 2', 'Reception', 'Year 7 mixed ability')",
          "flags": [
            "string (zero or more from: 'EHCP', 'SEN support', 'EAL', 'FSM', 'PP' (pupil premium), 'LAC' (looked-after child), 'YC' (young carer). Use only the abbreviations above. 0 to 4 flags per pupil. Most pupils carry 0 to 2 flags; a small number carry 3 or 4.)"
          ],
          "behaviour_note": "string or null (1 short sentence on a recent incident or support plan, or null if no behaviour pattern of concern. Anonymised. No graphic detail.)",
          "attendance_pattern": "string (a percentage with a short qualifier, e.g. '98% (steady)', '92% (declining over half term)', '78% (persistent absence)', '85% (Mondays consistently missed)')",
          "attainment_band": "string (one of: 'above', 'at', 'working_towards', 'below_age_related')",
          "priority_signal": "string (one of: 'immediate', 'monitor', 'routine'. Used to colour the row; 1 to 3 pupils per cohort should carry 'immediate'.)"
        }
      ]`
  }

  if (blockId === 'lesson-plan') {
    return base + `,
      "lesson_brief": {
        "subject": "string (a UK National Curriculum area, generic and anonymised: 'English (reading)', 'Maths (number)', 'Science (forces)', 'History (Tudor period)', 'PSHE (online safety)'. Never reference a real exam board specification.)",
        "year_group": "string (e.g. 'Year 4', 'Year 9 mixed ability', 'Reception', 'Year 11 Higher tier')",
        "duration_minutes": "number (40, 50, 60, or 75)",
        "topic": "string (a specific topic appropriate to the year group, e.g. 'Identifying figurative language in chapter 3', 'Adding fractions with the same denominator', 'Forces in motion: friction', 'Causes of the Reformation')",
        "class_context": "string (2-3 sentences on the class: how many pupils, the mix of SEN / EAL / HAP / LAP, recent assessment data, behaviour considerations, any specific names already flagged in the class roster from the previous block. Anonymised.)",
        "constraint": "string (one practical constraint that shapes the plan: 'cover for an absent colleague, no prior planning shared', 'shortened lesson because of an assembly', 'observed by a colleague for performance management', 'first lesson back after a difficult half term')",
        "scope": "string (one of: 'support_one_pupil' (TA), 'single_lesson' (Class Teacher), 'unit_sequence' (HoD: this lesson plus a 6-8 lesson sequence), 'review_colleague_plan' (SLT: review and feedback))"
      },
      "shared_materials": "string (3-5 sentences of the lesson context already shared by the teacher or HoD, written in their voice. Names the learning intention they have set, the prior lesson's outcome, any text or resource the candidate should plan around. For TA scope, this is the teacher's brief to the TA. For HoD scope, this is the existing scheme of work the HoD is sequencing within. For SLT scope, this is the colleague's plan being reviewed.)",
      "planted_considerations": [
        "string (3 to 5 specific considerations the candidate should weigh — used for scoring, not shown verbatim. Examples: 'Two pupils with EHCP need scaffolded sentence stems', 'Half the class are still recovering from the prior unit; pace must allow for retrieval practice', 'No mid-lesson AfL checkpoint shared in the existing plan', 'Resources rely on a worksheet that may not differentiate enough for HAP'.)"
      ]`
  }

  if (blockId === 'parent-communication') {
    return base + `,
      "channel": "string (one of: 'email', 'note_via_pupil', 'voicemail_transcript', 'message_on_school_app')",
      "parent_message": {
        "from_name": "string (a plausible UK first name + surname, e.g. 'Sarah Whitfield', 'Marcus Patel', 'Linda O'Connor'. Not a real public figure.)",
        "relationship_to_pupil": "string (e.g. 'Mother of Pupil A in 4B', 'Father of the new Year 7 starter', 'Step-parent of Pupil C', 'Carer of Pupil D, Year 9')",
        "subject_line": "string (the subject line as written by the parent: 'About yesterday', 'Concern about behaviour', 'My EHCP request', 'Following up on our conversation')",
        "received_at": "string (a believable timestamp this week, e.g. 'Yesterday, 19:42', 'This morning, 06:03', 'Friday afternoon, 16:20')",
        "full_text": "string (200-400 words in the parent or carer's voice. Lands like a real message: emotional content, specific incidents, sometimes contradictory or partial information, sometimes a request, sometimes a complaint, sometimes anxious. Anonymises pupil reference. Never includes graphic safeguarding content; if a safeguarding concern is hinted at, it is hinted at — not described in detail.)",
        "tone": "string (one of: 'anxious', 'angry', 'disappointed', 'curious', 'urgent', 'pleading', 'formal_complaint')"
      },
      "candidate_role_in_response": "string (1 sentence describing how the candidate is positioned to respond, role-specific. TA: 'Teacher has asked you to draft a response for them to review.' Class Teacher: 'You will respond directly within 24 hours.' SENCO: 'You are the named contact for this family on EHCP matters.' DSL/Head: 'You hold the safeguarding lead role and the parent has escalated to you.')",
      "safeguarding_signals": [
        "string (zero or more anonymised, professional indicators of a possible safeguarding-adjacent issue in the message — used for scoring only, not shown to the candidate. Empty array is valid for purely operational queries. Examples: 'Parent describes a pattern of pupil withdrawal at home over recent weeks', 'Message references an unexplained change in routine that may warrant follow-up', 'Parent names a third party in a way that suggests a contact restriction may apply'.)"
      ],
      "expected_copy_in": [
        "string (1 to 4 colleagues the candidate should consider copying or briefing on the response. Examples: 'SENCO', 'DSL', 'Headteacher', 'Year leader', 'Class Teacher', 'Pastoral Lead'. Used for scoring only.)"
      ]`
  }

  if (blockId === 'behaviour-incident') {
    return base + `,
      "incident_setting": "string (where and when, e.g. 'Year 4 lesson, mid-morning', 'Year 9 corridor between periods 3 and 4', 'Tuesday afternoon, Year 11 mock exam hall', 'Reception area at end of school')",
      "trigger_event": "string (2-3 sentences describing what just happened. Specific, anonymised, never graphic. Names pupils as 'Pupil A', 'Pupil B in 7C', 'two Year 6 pupils'. Captures what the candidate just saw or was told.)",
      "pupils_involved": [
        {
          "pupil_id": "string (unique within block, e.g. pupil-a)",
          "anonymised_label": "string (e.g. 'Pupil A', 'Pupil B', 'Year 9 pupil at the back')",
          "year_group": "string (e.g. 'Year 4', 'Year 9')",
          "relevant_context": "string (1 sentence of role-relevant context, e.g. 'EHCP for SEMH needs', 'Recently bereaved, on pastoral plan', 'New starter, transition file not yet arrived', 'Fluent EAL Stage 2'. Anonymised.)"
        }
      ],
      "other_pupils_reaction": "string (1-2 sentences on what the rest of the class or group is doing while this is happening. Crucial pressure context.)",
      "time_pressure": "string (a short label of the binding pressure, e.g. 'Lesson continues for another 35 minutes', 'Senior leader walking past in 90 seconds', 'Bell rings in 4 minutes', 'Parents evening starting in the same room in 20 minutes')",
      "adult_to_be_informed": "string (the adult who needs to be told, role-only: 'the Class Teacher', 'the Year Leader', 'the SENCO', 'the on-call SLT', 'the DSL')",
      "policy_anchors": [
        "string (3 to 4 elements of the school behaviour or safeguarding policy that apply — used for scoring, not shown verbatim. Examples: 'Step-2 sanction is the maximum the candidate can apply at their level', 'Incident must be logged on the school behaviour system before end of day', 'SEND code requires reasonable adjustments to be considered before sanction', 'DSL must be notified if a pupil discloses anything during de-escalation'.)"
      ]`
  }

  if (blockId === 'safeguarding-referral') {
    return base + `,
      "scenario_summary": "string (1 to 2 sentence brief — neutral, professional tone. Names the role context (a TA noticing, a teacher receiving a disclosure, a SENCO weighing a referral, a DSL handling a referral up the chain). No graphic language.)",
      "full_scenario_text": "string (3 to 5 sentences of anonymised detail. Pupil referenced as 'Pupil A in 4B', 'the Year 7 pupil in your support group', 'a pupil from another class'. The concern is described in professional, non-graphic terms; the focus is on what the candidate noticed, what they were told, or what was passed up to them — and the judgement about pathway. NEVER includes graphic detail beyond what would be in a written record. References the UK safeguarding framework (Working Together to Safeguard Children, Keeping Children Safe in Education, the Designated Safeguarding Lead role).)",
      "who_observed_or_disclosed": "string (1 sentence naming the role and timing: 'the candidate noticed during a small-group reading session yesterday', 'a Year 3 TA reported during morning briefing', 'the pupil disclosed in conversation after the lesson', 'a referral arrived from a Year 5 teacher overnight')",
      "indicators_present": [
        "string (3 to 5 specific, professional, non-graphic indicators that may be present. Examples: 'Pattern of withdrawal in class over recent weeks', 'Disclosure made by the pupil but partially retracted', 'Unexplained absence pattern that aligns with home contact days', 'Sibling already known to social care', 'Recent change in home circumstances reported by the parent'. Used for scoring and shown to the candidate as part of the scenario context.)"
      ],
      "framework_reminders": [
        "string (2 to 3 short anchors the candidate should weigh. Examples: 'KCSIE Part 1 obligations apply to all staff', 'Threshold for Section 17 vs Section 47 referral', 'Confidentiality vs duty to share — DSL discretion'. Used for scoring, kept brief.)"
      ],
      "recording_systems_in_use": [
        "string (1 to 3 systems available, common UK options: 'CPOMS', 'MyConcern', 'paper-based safeguarding log', 'school MIS safeguarding module')"
      ],
      "threshold_factors": [
        "string (3 to 5 factors the scorer weighs when judging the candidate's threshold call. Used for scoring only, not shown to the candidate. Examples: 'Pattern over time rather than a single event', 'Disclosure made directly by the pupil', 'Capacity of the pupil to consent to information sharing', 'Risk of immediate harm if no action today', 'Earlier concerns already on the record'.)"
      ]`
  }

  if (blockId === 'head-teacher-message') {
    return base + `,
      "scope": "string (one of: 'respond_to_request' (TA / Class Teacher / SENCO / HoD / Pastoral Lead — the candidate replies to a head teacher message), 'draft_school_wide_message' (SLT / Headteacher — the candidate writes the message themselves))",
      "head_message": {
        "from_role": "string (the named-but-fictional sender role, e.g. 'Headteacher', 'Acting Headteacher', 'Executive Head of the Trust', 'MAT CEO'. Only used when scope is respond_to_request. For draft_school_wide_message this carries the role the candidate is writing AS, e.g. 'Headteacher'.)",
        "subject_line": "string (a believable internal subject, e.g. 'Year 6 SATs results — your view by 14:00', 'Friday all-staff briefing — content for your section', 'Following up on yesterday's pastoral concern', 'Trust-wide cover plan request')",
        "sent_at": "string (a believable timestamp, e.g. 'Today 08:50', 'Yesterday 16:42', 'Friday 07:18')",
        "deadline": "string (the binding deadline on the candidate, e.g. 'by 11:00 today', 'before the staff briefing on Monday', 'before the governors call at 17:00', or 'no fixed deadline' if genuinely open)",
        "audience": "string (who the candidate is writing TO. For respond_to_request: 'the headteacher and one named copy-in'. For draft_school_wide_message: 'all staff', 'parent body', 'the chair of governors', 'the MAT CEO and trust leadership'.)",
        "full_text": "string (200-450 words. For respond_to_request, this is the head's message in their voice — a directive, a query, a briefing request, or a sensitive update — referencing year groups, anonymised pupils, named-but-fictional staff roles. For draft_school_wide_message, this is the brief the candidate has been given to write FROM: the situation, the ask, and the constraint. Anonymised. No real schools, no real OFSTED references — use 'an inspection', 'the regulator', 'the LA'.)",
        "issue_type": "string (one of: 'governance_inquiry', 'inspection_or_regulator_prep', 'mat_or_trust_priority', 'parent_body_communication', 'pastoral_or_safeguarding_followup', 'data_or_results_request', 'cover_or_resourcing', 'crisis_briefing')"
      },
      "strategic_anchors": [
        "string (3 to 5 strategic factors the candidate should weigh — used for scoring, not rendered verbatim. Examples: 'Tone must reassure without overcommitting', 'Governor expects accountability framing, not operational detail', 'Safeguarding line cannot be inadvertently disclosed in a school-wide message', 'MAT CEO will read this against trust strategic priorities, not local context'.)"
      ]`
  }

  if (blockId === 'cohort-coordination') {
    return base + `,
      "scope": "string (one of: 'small_group' (TA — coordinate support across a few pupils in a single lesson), 'single_class_or_year' (Class Teacher — coordinate a year group event, exam prep, intervention), 'cross_class_or_department' (SENCO / HoD / Pastoral Lead — coordinate across classes or department), 'whole_school' (SLT / Headteacher — whole-school or multi-site coordination, e.g. cover crisis, inspection prep, MAT-wide))",
      "coordination_window": "string (the binding window, e.g. 'Tuesday morning 09:00-12:00', 'today through Friday', 'next Monday', 'next 48 hours')",
      "demands": [
        {
          "demand_id": "string (unique within block, e.g. demand-1)",
          "title": "string (5 to 12 words naming the specific operational demand, e.g. 'Year 11 mock exam invigilation', 'Year 4 EHCP review meeting', 'Cover for two staff sickness', 'Governor visit walkthrough', 'Parent body briefing on results')",
          "stakeholder": "string (the named role asking, e.g. 'Year 6 lead', 'Chair of governors', 'SENCO', 'On-call SLT', 'MAT operations director')",
          "deadline": "string (when this lands, e.g. '11:00 today', 'end of school day', 'tomorrow morning briefing', 'Friday 16:00')",
          "constraint": "string (1 sentence on the binding constraint, e.g. 'Two invigilators short, exam cannot start late', 'Parent must be told before they collect their child', 'Cover already pulled from PPA, no further capacity', 'Governor expects the head, not a deputy')",
          "priority_signal": "string (one of: 'critical', 'high', 'medium', 'low'. Critical = inspection, safeguarding, safety. High = exam, parent meeting, deadline. Medium = routine but time-bound. Low = nice-to-have.)"
        }
      ],
      "resource_constraints": [
        "string (3 to 5 specific resource constraints in play, e.g. 'Two staff signed off sick today', 'PPA cover already used for another absence', 'Hall booked for the rehearsal until 11:30', 'Budget for additional cover already at limit', 'On-call SLT is in a parent meeting until 10:30'.)"
      ],
      "escalation_anchors": [
        "string (2 to 4 escalation anchors the scorer weighs — used for scoring, not rendered verbatim. Examples: 'Critical demand without a named decision-maker should escalate to SLT', 'Safeguarding-adjacent demand cannot be deprioritised even when high-volume', 'Parent communication carrying legal weight needs head sign-off'.)"
      ]`
  }

  if (blockId === 'conversation-simulation') {
    return base + `,
      "counterpart_type": "string (one of: 'parent', 'pupil', 'colleague', 'governor', 'mat_or_la_representative')",
      "counterpart_persona": {
        "name": "string (plausible UK first + surname, fictional, e.g. 'Helen Marsh', 'Daniel Okafor', 'Priya Shah', 'Alan Reid'. Never a real public figure.)",
        "relationship_to_situation": "string (concrete, e.g. 'Mother of Pupil A in 4B', 'Year 9 pupil in your form group', 'Year 5 colleague pushing back on a referral', 'Chair of Governors raising the inspection concern', 'LA SEND officer questioning EHCP timing')",
        "emotional_state": "string (1 sentence on their state at the start, e.g. 'angry and threatening to escalate to the LA', 'anxious and asking the same question different ways', 'professionally polite but pushing hard on the underlying issue', 'distressed and finding it hard to stay on topic')",
        "core_position": "string (1 sentence on the position they are holding to, in their voice)",
        "what_lands_with_them": "string (1 sentence on what genuinely lands, e.g. 'A specific commitment with a named person and time', 'Acknowledgement of the past failure before any reassurance', 'A clear next step they can describe back to their family', 'Hearing the safeguarding line cited explicitly')",
        "how_they_push_back": "string (1 sentence on the move they make when reassured but not satisfied, e.g. 'Repeats the same point with new framing', 'Names a specific incident as evidence', 'Threatens to escalate to OFSTED / LA / governors', 'Goes quiet and then returns sharper')"
      },
      "conversation_setting": "string (one of: 'at the school gate', 'at parents evening', 'in your classroom after school', 'in the head office', 'in the staffroom', 'on the phone', 'on a video call')",
      "purpose": "string (one of: 'parent_complaint_response', 'pupil_supportive_conversation', 'colleague_pushback', 'governor_or_mat_inquiry', 'la_or_external_inquiry')",
      "turns": [
        {
          "turn_index": "number (1, 2, or 3 — fixed three-turn structure)",
          "counterpart_says": "string (2 to 4 sentences the counterpart opens this turn with. Lands like real speech in their voice. Each turn escalates or shifts pressure: turn 1 sets the position, turn 2 pushes back on a likely response, turn 3 forces a definitive call.)",
          "what_this_turn_tests": "string (one sentence describing what the candidate's response is being assessed against, e.g. 'Acknowledgement before reassurance', 'Holding a boundary without breaking rapport', 'Naming the safeguarding line without disclosing detail'. Used for scoring, not shown to the candidate.)"
        }
      ],
      "regulation_anchors": [
        "string (2 to 4 emotional regulation and de-escalation indicators the scorer weighs — used for scoring only. Examples: 'Pace stays slow even as counterpart speeds up', 'Names a feeling before naming an action', 'Resists explaining when listening would land better', 'Holds the safeguarding line without escalating affect'.)"
      ]`
  }

  if (blockId === 'crisis-simulation') {
    return base + `,
      "scenario_summary": "string (1 to 2 sentence brief — neutral, professional tone. Names the role context (a TA in a lesson, a Class Teacher in their classroom, a Pastoral Lead taking an escalation, an SLT or Head taking a call). No graphic language.)",
      "full_scenario_text": "string (3 to 5 sentences of anonymised detail. References pupils as 'Pupil A in 4B', 'a Year 9 pupil', 'a member of the Year 11 cohort'. The crisis is described in professional, non-graphic terms; the focus is on what the candidate has just been told and the immediate decisions in front of them. NEVER includes graphic detail. References the UK safeguarding and crisis-management framework where relevant (KCSIE, the DSL role, safeguarding partners, LA escalation).)",
      "trigger_type": "string (one of: 'pupil_distress_in_lesson', 'classroom_incident_requiring_intervention', 'escalation_from_colleague', 'mental_health_emergency', 'family_or_disclosure_crisis', 'school_wide_safeguarding_incident', 'media_or_regulator_inquiry')",
      "who_alerted_the_candidate": "string (1 sentence naming who, when, and how, e.g. 'A Year 4 TA put their head round the door at 10:42', 'The school office took a call from a parent at 14:05', 'The on-call duty phone rang during your meeting at 09:18', 'A Year 11 pupil disclosed in the corridor between periods')",
      "immediate_pressures": [
        "string (3 to 5 specific pressures in play, e.g. 'Lesson continues for another 25 minutes with 28 other pupils in the room', 'Parent expecting a call back within 10 minutes', 'On-call SLT already managing a separate incident', 'Local press already asking for a statement', 'Pupil at risk is currently unsupervised'.)"
      ],
      "framework_reminders": [
        "string (2 to 3 short framework anchors the candidate should weigh, e.g. 'KCSIE Part 1 obligations', 'DSL must be informed before parents are contacted on safeguarding matters', 'LADO threshold for staff-conduct concerns', 'Critical incident plan applies once the crisis is school-wide'. Used for scoring, kept brief.)"
      ],
      "stages": [
        {
          "stage_index": "number (1, 2, or 3 — fixed three-stage escalation: stage 1 is the initial call to act, stage 2 is the situation shifting under the candidate's response, stage 3 is the final pressure point)",
          "new_information": "string (2 to 4 sentences on what the candidate now knows, in professional, non-graphic terms. Stage 1 = what just happened. Stage 2 = how it has shifted (a senior figure walking in, a parent calling, a second pupil affected, a deadline closing). Stage 3 = the final escalation (a regulator or LA contact, a media question, a safeguarding partner asking for a decision).)",
          "decision_prompt": "string (the specific question the candidate must answer at this stage, e.g. 'What do you do in the next 60 seconds and who do you call first?', 'How do you respond to the parent now on the phone?', 'What do you tell the regulator and what do you hold back until you have spoken to the DSL?')"
        }
      ],
      "post_crisis_anchors": [
        "string (2 to 4 post-crisis follow-up indicators the scorer weighs — used for scoring only. Examples: 'Pupils witnessing the incident need a debrief plan', 'Staff involved need a wellbeing check-in within 24 hours', 'CPOMS / safeguarding log entry must reflect both the event and the response', 'External communications log must be opened for any media or regulator contact'.)"
      ]`
  }
...
// Per-block typed-field normalisers for the education shell. Strict
// enough that the live UI can trust the shapes; permissive enough that
// an old cached scenario or a model response that drops a field does
// not crash the orchestrator. Mirrors the healthcare normaliser
// pattern (normaliseHealthcarePatients, normaliseHealthcareSafeguarding).
function normaliseEducationPupils(arr) {
  if (!Array.isArray(arr)) return null
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const FLAGS = ['EHCP', 'SEN support', 'EAL', 'FSM', 'PP', 'LAC', 'YC']
  const ATTAINMENT = ['above', 'at', 'working_towards', 'below_age_related']
  const PRIORITY = ['immediate', 'monitor', 'routine']
  const safeFlags = (v) => Array.isArray(v)
    ? Array.from(new Set(v.map(safeStr))).filter(f => FLAGS.includes(f)).slice(0, 4)
    : []
  return arr.slice(0, 30).map((p, i) => ({
    pupil_id: typeof p?.pupil_id === 'string' && p.pupil_id ? p.pupil_id : `pupil-${i + 1}`,
    anonymised_name: safeStr(p?.anonymised_name) || `Pupil ${i + 1}`,
    year_group_or_class: safeStr(p?.year_group_or_class),
    flags: safeFlags(p?.flags),
    behaviour_note: safeStr(p?.behaviour_note) || null,
    attendance_pattern: safeStr(p?.attendance_pattern),
    attainment_band: ATTAINMENT.includes(p?.attainment_band) ? p.attainment_band : 'at',
    priority_signal: PRIORITY.includes(p?.priority_signal) ? p.priority_signal : 'routine',
  })).filter(p => p.anonymised_name)
}

function normaliseEducationLessonPlan(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const SCOPES = ['support_one_pupil', 'single_lesson', 'unit_sequence', 'review_colleague_plan']
  const briefRaw = entry?.lesson_brief || {}
  const lesson_brief = {
    subject: safeStr(briefRaw?.subject),
    year_group: safeStr(briefRaw?.year_group),
    duration_minutes: Number.isFinite(briefRaw?.duration_minutes) ? briefRaw.duration_minutes : 50,
    topic: safeStr(briefRaw?.topic),
    class_context: safeStr(briefRaw?.class_context),
    constraint: safeStr(briefRaw?.constraint),
    scope: SCOPES.includes(briefRaw?.scope) ? briefRaw.scope : 'single_lesson',
  }
  const shared_materials = safeStr(entry?.shared_materials)
  const planted_considerations = safeArr(entry?.planted_considerations)
  if (!lesson_brief.topic && !shared_materials) return null
  return { lesson_brief, shared_materials, planted_considerations }
}

function normaliseEducationParentMessage(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const CHANNELS = ['email', 'note_via_pupil', 'voicemail_transcript', 'message_on_school_app']
  const TONES = ['anxious', 'angry', 'disappointed', 'curious', 'urgent', 'pleading', 'formal_complaint']
  const channel = CHANNELS.includes(entry?.channel) ? entry.channel : 'email'
  const msgRaw = entry?.parent_message || {}
  const parent_message = {
    from_name: safeStr(msgRaw?.from_name) || 'Parent',
    relationship_to_pupil: safeStr(msgRaw?.relationship_to_pupil),
    subject_line: safeStr(msgRaw?.subject_line),
    received_at: safeStr(msgRaw?.received_at),
    full_text: safeStr(msgRaw?.full_text),
    tone: TONES.includes(msgRaw?.tone) ? msgRaw.tone : 'anxious',
  }
  if (!parent_message.full_text || parent_message.full_text.length < 80) return null
  return {
    channel,
    parent_message,
    candidate_role_in_response: safeStr(entry?.candidate_role_in_response),
    safeguarding_signals: safeArr(entry?.safeguarding_signals),
    expected_copy_in: safeArr(entry?.expected_copy_in),
  }
}

function normaliseEducationBehaviourIncident(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const trigger_event = safeStr(entry?.trigger_event)
  if (!trigger_event || trigger_event.length < 40) return null
  const pupilsRaw = Array.isArray(entry?.pupils_involved) ? entry.pupils_involved : []
  const pupils_involved = pupilsRaw.slice(0, 4).map((p, i) => ({
    pupil_id: typeof p?.pupil_id === 'string' && p.pupil_id ? p.pupil_id : `pupil-${i + 1}`,
    anonymised_label: safeStr(p?.anonymised_label) || `Pupil ${String.fromCharCode(65 + i)}`,
    year_group: safeStr(p?.year_group),
    relevant_context: safeStr(p?.relevant_context),
  })).filter(p => p.anonymised_label)
  return {
    incident_setting: safeStr(entry?.incident_setting),
    trigger_event,
    pupils_involved,
    other_pupils_reaction: safeStr(entry?.other_pupils_reaction),
    time_pressure: safeStr(entry?.time_pressure),
    adult_to_be_informed: safeStr(entry?.adult_to_be_informed),
    policy_anchors: safeArr(entry?.policy_anchors),
  }
}

function normaliseEducationSafeguarding(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const full_scenario_text = safeStr(entry?.full_scenario_text)
  if (!full_scenario_text || full_scenario_text.length < 80) return null
  return {
    scenario_summary: safeStr(entry?.scenario_summary),
    full_scenario_text,
    who_observed_or_disclosed: safeStr(entry?.who_observed_or_disclosed),
    indicators_present: safeArr(entry?.indicators_present),
    framework_reminders: safeArr(entry?.framework_reminders),
    recording_systems_in_use: safeArr(entry?.recording_systems_in_use),
    threshold_factors: safeArr(entry?.threshold_factors),
  }
}

function normaliseEducationHeadTeacherMessage(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const SCOPES = ['respond_to_request', 'draft_school_wide_message']
  const ISSUE_TYPES = [
    'governance_inquiry', 'inspection_or_regulator_prep', 'mat_or_trust_priority',
    'parent_body_communication', 'pastoral_or_safeguarding_followup',
    'data_or_results_request', 'cover_or_resourcing', 'crisis_briefing',
  ]
  const scope = SCOPES.includes(entry?.scope) ? entry.scope : 'respond_to_request'
  const msgRaw = entry?.head_message || {}
  const head_message = {
    from_role: safeStr(msgRaw?.from_role) || 'Headteacher',
    subject_line: safeStr(msgRaw?.subject_line),
    sent_at: safeStr(msgRaw?.sent_at),
    deadline: safeStr(msgRaw?.deadline),
    audience: safeStr(msgRaw?.audience),
    full_text: safeStr(msgRaw?.full_text),
    issue_type: ISSUE_TYPES.includes(msgRaw?.issue_type) ? msgRaw.issue_type : 'governance_inquiry',
  }
  if (!head_message.full_text || head_message.full_text.length < 80) return null
  return {
    scope,
    head_message,
    strategic_anchors: safeArr(entry?.strategic_anchors),
  }
}

function normaliseEducationCohortCoordination(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const SCOPES = ['small_group', 'single_class_or_year', 'cross_class_or_department', 'whole_school']
  const PRIORITIES = ['critical', 'high', 'medium', 'low']
  const demandsRaw = Array.isArray(entry?.demands) ? entry.demands : []
  const demands = demandsRaw.slice(0, 8).map((d, i) => ({
    demand_id: typeof d?.demand_id === 'string' && d.demand_id ? d.demand_id : `demand-${i + 1}`,
    title: safeStr(d?.title) || `Demand ${i + 1}`,
    stakeholder: safeStr(d?.stakeholder),
    deadline: safeStr(d?.deadline),
    constraint: safeStr(d?.constraint),
    priority_signal: PRIORITIES.includes(d?.priority_signal) ? d.priority_signal : 'medium',
  })).filter(d => d.title && d.constraint)
  if (demands.length < 3) return null
  return {
    scope: SCOPES.includes(entry?.scope) ? entry.scope : 'single_class_or_year',
    coordination_window: safeStr(entry?.coordination_window),
    demands,
    resource_constraints: safeArr(entry?.resource_constraints),
    escalation_anchors: safeArr(entry?.escalation_anchors),
  }
}

function normaliseEducationConversationSimulation(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const TYPES = ['parent', 'pupil', 'colleague', 'governor', 'mat_or_la_representative']
  const PURPOSES = [
    'parent_complaint_response', 'pupil_supportive_conversation',
    'colleague_pushback', 'governor_or_mat_inquiry', 'la_or_external_inquiry',
  ]
  const counterpart_type = TYPES.includes(entry?.counterpart_type) ? entry.counterpart_type : 'parent'
  const personaRaw = entry?.counterpart_persona || {}
  const counterpart_persona = {
    name: safeStr(personaRaw?.name) || 'Counterpart',
    relationship_to_situation: safeStr(personaRaw?.relationship_to_situation),
    emotional_state: safeStr(personaRaw?.emotional_state),
    core_position: safeStr(personaRaw?.core_position),
    what_lands_with_them: safeStr(personaRaw?.what_lands_with_them),
    how_they_push_back: safeStr(personaRaw?.how_they_push_back),
  }
  const turnsRaw = Array.isArray(entry?.turns) ? entry.turns : []
  const turns = turnsRaw.slice(0, 3).map((t, i) => ({
    turn_index: Number.isFinite(t?.turn_index) ? Math.max(1, Math.min(3, Math.round(t.turn_index))) : (i + 1),
    counterpart_says: safeStr(t?.counterpart_says),
    what_this_turn_tests: safeStr(t?.what_this_turn_tests),
  })).filter(t => t.counterpart_says && t.counterpart_says.length >= 30)
  if (turns.length < 3) return null
  return {
    counterpart_type,
    counterpart_persona,
    conversation_setting: safeStr(entry?.conversation_setting),
    purpose: PURPOSES.includes(entry?.purpose) ? entry.purpose : 'parent_complaint_response',
    turns,
    regulation_anchors: safeArr(entry?.regulation_anchors),
  }
}

function normaliseEducationCrisisSimulation(entry) {
  const safeStr = (v) => typeof v === 'string' ? v.trim() : ''
  const safeArr = (v) => Array.isArray(v) ? v.map(safeStr).filter(Boolean).slice(0, 6) : []
  const TRIGGERS = [
    'pupil_distress_in_lesson', 'classroom_incident_requiring_intervention',
    'escalation_from_colleague', 'mental_health_emergency',
    'family_or_disclosure_crisis', 'school_wide_safeguarding_incident',
    'media_or_regulator_inquiry',
  ]
  const full_scenario_text = safeStr(entry?.full_scenario_text)
  if (!full_scenario_text || full_scenario_text.length < 80) return null
  const stagesRaw = Array.isArray(entry?.stages) ? entry.stages : []
  const stages = stagesRaw.slice(0, 3).map((s, i) => ({
    stage_index: Number.isFinite(s?.stage_index) ? Math.max(1, Math.min(3, Math.round(s.stage_index))) : (i + 1),
    new_information: safeStr(s?.new_information),
    decision_prompt: safeStr(s?.decision_prompt),
  })).filter(s => s.new_information && s.decision_prompt)
  if (stages.length < 3) return null
  return {
    scenario_summary: safeStr(entry?.scenario_summary),
    full_scenario_text,
    trigger_type: TRIGGERS.includes(entry?.trigger_type) ? entry.trigger_type : 'classroom_incident_requiring_intervention',
    who_alerted_the_candidate: safeStr(entry?.who_alerted_the_candidate),
    immediate_pressures: safeArr(entry?.immediate_pressures),
    framework_reminders: safeArr(entry?.framework_reminders),
    stages,
    post_crisis_anchors: safeArr(entry?.post_crisis_anchors),
  }
}
...
export async function generateEducationScenario(client, role_profile, { roleTitle, jobDescription, blockOverride } = {}) {
  ...
  for (const s of selected) {
    ...
    if (s.block_id === 'class-roster') {
      const pupils = normaliseEducationPupils(entry.pupils)
      if (pupils && pupils.length) {
        blockShape.roster_label = safeStr(entry.roster_label)
        blockShape.roster_context = safeStr(entry.roster_context)
        blockShape.pupils = pupils
      }
    } else if (s.block_id === 'lesson-plan') {
      const lp = normaliseEducationLessonPlan(entry)
      if (lp) {
        blockShape.lesson_brief = lp.lesson_brief
        blockShape.shared_materials = lp.shared_materials
        blockShape.planted_considerations = lp.planted_considerations
      }
    } else if (s.block_id === 'parent-communication') {
      const pc = normaliseEducationParentMessage(entry)
      if (pc) {
        blockShape.channel = pc.channel
        blockShape.parent_message = pc.parent_message
        blockShape.candidate_role_in_response = pc.candidate_role_in_response
        blockShape.safeguarding_signals = pc.safeguarding_signals
        blockShape.expected_copy_in = pc.expected_copy_in
      }
    } else if (s.block_id === 'behaviour-incident') {
      const bi = normaliseEducationBehaviourIncident(entry)
      if (bi) {
        blockShape.incident_setting = bi.incident_setting
        blockShape.trigger_event = bi.trigger_event
        blockShape.pupils_involved = bi.pupils_involved
        blockShape.other_pupils_reaction = bi.other_pupils_reaction
        blockShape.time_pressure = bi.time_pressure
        blockShape.adult_to_be_informed = bi.adult_to_be_informed
        blockShape.policy_anchors = bi.policy_anchors
      }
    } else if (s.block_id === 'safeguarding-referral') {
      const sg = normaliseEducationSafeguarding(entry)
      if (sg) {
        blockShape.scenario_summary = sg.scenario_summary
        blockShape.full_scenario_text = sg.full_scenario_text
        blockShape.who_observed_or_disclosed = sg.who_observed_or_disclosed
        blockShape.indicators_present = sg.indicators_present
        blockShape.framework_reminders = sg.framework_reminders
        blockShape.recording_systems_in_use = sg.recording_systems_in_use
        blockShape.threshold_factors = sg.threshold_factors
      }
    } else if (s.block_id === 'head-teacher-message') {
      const hm = normaliseEducationHeadTeacherMessage(entry)
      if (hm) {
        blockShape.scope = hm.scope
        blockShape.head_message = hm.head_message
        blockShape.strategic_anchors = hm.strategic_anchors
      }
    } else if (s.block_id === 'cohort-coordination') {
      const cc = normaliseEducationCohortCoordination(entry)
      if (cc) {
        blockShape.scope = cc.scope
        blockShape.coordination_window = cc.coordination_window
        blockShape.demands = cc.demands
        blockShape.resource_constraints = cc.resource_constraints
        blockShape.escalation_anchors = cc.escalation_anchors
      }
    } else if (s.block_id === 'conversation-simulation') {
      const cs = normaliseEducationConversationSimulation(entry)
      if (cs) {
        blockShape.counterpart_type = cs.counterpart_type
        blockShape.counterpart_persona = cs.counterpart_persona
        blockShape.conversation_setting = cs.conversation_setting
        blockShape.purpose = cs.purpose
        blockShape.turns = cs.turns
        blockShape.regulation_anchors = cs.regulation_anchors
      }
    } else if (s.block_id === 'crisis-simulation') {
      const cr = normaliseEducationCrisisSimulation(entry)
      if (cr) {
        blockShape.scenario_summary = cr.scenario_summary
        blockShape.full_scenario_text = cr.full_scenario_text
        blockShape.trigger_type = cr.trigger_type
        blockShape.who_alerted_the_candidate = cr.who_alerted_the_candidate
        blockShape.immediate_pressures = cr.immediate_pressures
        blockShape.framework_reminders = cr.framework_reminders
        blockShape.stages = cr.stages
        blockShape.post_crisis_anchors = cr.post_crisis_anchors
      }
    }
    block_content[s.block_id] = blockShape
  }
