...
function buildScenarioPrompt({ role_profile, roleTitle, jobDescription, selected, matchInfo }) {
  ...
  return `You are designing a connected 9am Monday Workspace simulation for a candidate applying for the role below. The candidate will spend 13 to 23 minutes (depending on block count) working through a sequence of blocks. The blocks are ALREADY chosen and ordered for you. Your job is to write a believable scenario that flows through them, with each block's content connecting to the previous block's output.
...
CRITICAL: every block's content must reflect the actual function (${role_profile.function}) and the sector context, not just the seniority. Two candidates at the same level in different functions must see materially different content.
${dataFlavour ? `For this function, data-heavy blocks (spreadsheet-data, reading-summarising) should focus on: ${dataFlavour}.` : ''}
For document-writing blocks, the document type must match the function: a Marketing Manager writes a campaign brief, a Solicitor drafts a letter or contract clause, a Finance Manager writes a variance commentary or board update, a Software Developer writes a design doc or post-incident note, a Customer Service Manager writes a complaint response, an HR Manager writes a grievance outcome letter.
For decision-queue blocks, the decisions must be ones this specific role would actually face on a real Monday: budget approvals for marketing, refund authority for customer service, fee discount calls for sales, hire/no-hire for recruitment, settlement vs litigate for legal, hotfix vs proper fix for software, etc.
For conversation-simulation blocks, name the counter-party explicitly and give them a position the candidate has to engage with (a procurement contact pushing for a discount, a line manager refusing to performance-manage, a board member raising a governance concern, etc.).
