...
//
// Phase 2 Education v0.5: 5 of 9 blocks are real interactive components
// (class-roster, lesson-plan, parent-communication, behaviour-incident,
// safeguarding-referral). The remaining 4 (head-teacher-message,
// cohort-coordination, conversation-simulation, crisis-simulation)
// still share the office BlockPlaceholder until the next prompt; the
// `is_real` flag below records the status so the admin harness and the
// orchestrator can distinguish real from stub without importing JSX on
// the server.
//
// scoring_signals are placeholders today and will be sharpened when
// each real block scorer ships.
