...
// Clinical crisis simulation, healthcare shell.
//
// Mirrors the architecture of the office crisis-simulation block (intro
// panel with a pulsing jade alert, three escalating stages, reflection
// at the end). Adds two trigger types that fit clinical and care
// contexts: buzzer_alert and walk_in. Stage content is healthcare-
// flavoured via the scenario generator's prompt rules.
//
// Block timer is informational only; the master countdown lives at the
// orchestrator level.
//
// Compliance:
//   - The block UI never renders a real drug name or dose. The scenario
//     generator's prompt forbids them.
//   - Pulsing border on the alert uses jade, not red. No traffic-light
//     tones anywhere in the block.
//   - Reflection copy stays grave and professional.
...
const TRIGGER_LABELS = {
  ...
  buzzer_alert:  { tag: 'Buzzer alert',   verb: 'Respond to the buzzer' },
  walk_in:       { tag: 'Walk-in',        verb: 'See the visitor' },
}
...
export default function ClinicalCrisisSimulationBlock({ role_profile, block_content, scenario_context, onComplete }) {
    ...
    return (
      <div style={{ fontFamily: F }}>
        <BlockScenarioHeader
          ...
          blockName="Clinical crisis simulation"
        />
        <NoCrisisFallback
          block_content={block_content}
          onComplete={() => onComplete && onComplete({
            block_id: 'clinical-crisis-simulation',
            fallback: true,
  ...
  return (
    <div style={{ fontFamily: F }}>
      ...
      {stageIndex === -1 ? (
        <IntroPanel crisis={crisis} isMobile={isMobile} onStart={handleStart} />
      ) : null}
...
function IntroPanel({ crisis, isMobile, onStart }) {
  ...
  return (
    <div className="prodicta-crisis-slide" style={{ maxWidth: 760, margin: '0 auto 18px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {sender.role ? (
            <div style={{ fontFamily: F, fontSize: 14, color: '#475569', marginTop: 2 }}>
              ...
              {sender.relationship ? (
                <span style={{
                  fontFamily: FM, fontSize: 11, color: '#64748b', marginLeft: 8,
                  textTransform: 'uppercase', letterSpacing: '0.06em',
                }}>{sender.relationship}</span>
              ) : null}
            </div>
          ) : null}
        </div>
  ...
  return (
    <div className="prodicta-crisis-slide" style={{ maxWidth: 760, margin: '0 auto 18px' }}>
      ...
      <textarea
        ...
        rows={isMobile ? 5 : 6}
        placeholder="Type what you say or do. Who do you call, what is the first sentence, what do you commit to, what do you escalate?"
        style={{
  ...
  return (
    <div className="prodicta-crisis-slide" style={{
      ...
      <div style={{
        ...
        Crisis handled · {completed} of {totalStages} stages
      </div>
...
function buildTranscript(crisis, responses) {
  ...
  for (let i = 0; i < responses.length; i++) {
    ...
    if (stage?.new_information) lines.push(`Update (stage ${i + 1}): ${stage.new_information}`)
    if (stage?.prompt) lines.push(`Prompt: ${stage.prompt}`)
