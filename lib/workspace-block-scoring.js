...
// future is one line here plus one import above. Office and healthcare
// blocks share the same dispatch dict because block_id is unique
// across shells (no collisions today and none expected — names are
// deliberately shell-flavoured).
// ─────────────────────────────────────────────────────────────────────────
const SCORERS = {
  // Office shell
  'inbox':                       scoreInbox,
...
// Set of block_ids that belong to the healthcare shell. Used by the
// audit-trail logic to pick the right block_library_version constant.
const HEALTHCARE_BLOCK_IDS = new Set([
  'patient-handover',
  'buzzer-alert-queue',
  'medication-round',
...
function blockLibraryVersionFor(assessment, blockScores) {
  ...
  // scored. If any healthcare block_id is present, treat as healthcare.
  if (Array.isArray(blockScores) && blockScores.some(b => HEALTHCARE_BLOCK_IDS.has(b.block_id))) {
...
async function synthesiseNarrative({ anthropic, assessment, role_title, role_profile, account_type, employment_type, workspace_score, blockScores }) {
