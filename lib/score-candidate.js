...

{
  ...
  // Extract between first { and last } in case there's surrounding text
  ...
  try {
    ...
    console.error('[score] JSON parse failed:', parseErr?.message)
    console.error('[score] Raw response preview (first 500 chars):', content.slice(0, 500))
    console.error('[score] Cleaned JSON preview (first 500 chars):', jsonStr.slice(0, 500))
