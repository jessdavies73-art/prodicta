...
${resp ? resp.response_text : '[No response provided]'}${fcBlock}`
  }).join('\n\n---\n\n')
...

{
  ...
  "scoring_confidence": {
    "level": "medium",
    "reason": "One sentence explaining why this level of confidence, referencing response depth and dimension consistency."
  },
...
You are generating targeted interview verification questions for a hiring manager or recruiter.

These questions are NOT generic competency questions. Each question must be directly linked to a specific insight from this candidate's assessment report (a watch-out, a medium-confidence dimension, a predicted outcome, or the top strength).

For each question generate three things:
1. The question itself, specific, behavioural, impossible to answer with a rehearsed response
2. A reassuring answer, what a strong, honest answer sounds like, with specific details, ownership language, and clear reasoning
3. A concerning answer, what a weak or evasive answer sounds like, with vague generalities, deflection, blame, or inconsistency with the assessment findings

Generate questions across these four categories:

CATEGORY 1, VERIFY TOP WATCH-OUTS (1 question per watch-out, maximum 3)
For each significant watch-out in the assessment generate a question that probes whether the watch-out is a real pattern or a one-off. Tag with verification_type: "watch_out" and the watch-out title as linked_to.

CATEGORY 2, VERIFY MODERATE CONFIDENCE DIMENSIONS (1 question per medium-confidence dimension, maximum 2)
For any dimension where the scoring confidence was medium or low, generate a question that gives the candidate a chance to demonstrate that dimension more fully. Tag with verification_type: "confidence_gap" and the dimension name as linked_to.

CATEGORY 3, VERIFY PREDICTED OUTCOMES (1 question maximum)
For the most significant predicted outcome (placement survival, pass probation probability, or counter-offer resilience) generate one question that tests whether the prediction holds. Tag with verification_type: "prediction" and the outcome name as linked_to.

CATEGORY 4, STRENGTHS CONFIRMATION (1 question maximum)
For the top strength generate one question that allows the candidate to demonstrate it with a real example from their history. Tag with verification_type: "strength_confirmation" and the strength name as linked_to.

Output must be a single JSON array (no prose, no markdown fences). Each array item has this exact shape:

{
  "question": "The interview question, 2 to 3 sentences, practical, role-specific",
  "verification_type": "watch_out" or "confidence_gap" or "prediction" or "strength_confirmation",
  "linked_to": "Name of the watch-out, dimension, outcome, or strength this question verifies",
  "reassuring_answer": "What a strong, honest answer sounds like, 2-3 sentences",
  "concerning_answer": "What a weak or evasive answer sounds like, 2-3 sentences",
  "confidence_level": "high" or "medium"
}

Rules:
- Generate between 4 and 8 items total.
- Prioritise watch_out and confidence_gap questions over prediction and strength_confirmation.
- Mark at least one watch_out question as confidence_level "high" if any watch-out exists.
- Write in UK English. No emoji. No em dashes. No en dashes.
- Do NOT wrap the output in markdown code fences. Return the JSON array directly, with nothing before or after.
...
Return JSON only. UK English. No emoji. No em dashes.
{
  ...
  const [narratives2aMsg, narratives2bMsg, narratives2cMsg, narratives2dMsg, narratives2eMsg, narratives2fMsg] = await Promise.all([
    ...
    // Call 2f: Workspace scoring (Strategy-Fit only)
...
Workspace data: ${JSON.stringify(wsResponses[0].workspace_data)}
...
{"workspace_score": 0-100, "workspace_narrative": "3-4 sentences", "workspace_signals": ["string"], "workspace_watch_out": "string or null"}` }],
  ...
  // Interview questions are now produced as a JSON array of structured objects
  // (verification mode). Fall back to the old "numbered list" format if the
  // model ignores the JSON instruction, so we never ship an empty section.
  const interviewRaw = getSection(text2b, 'INTERVIEW QUESTIONS')
  let interviewQuestions = []
  if (interviewRaw) {
    const stripped = interviewRaw.replace(/^```json?\s*/i, '').replace(/\s*```$/i, '').trim()
    const firstBracket = stripped.indexOf('[')
    const lastBracket = stripped.lastIndexOf(']')
    if (firstBracket !== -1 && lastBracket > firstBracket) {
      try {
        const parsed = JSON.parse(stripped.slice(firstBracket, lastBracket + 1).replace(/[—–]/g, ','))
        if (Array.isArray(parsed) && parsed.length > 0) {
          interviewQuestions = parsed
            .filter(q => q && typeof q === 'object' && typeof q.question === 'string')
            .map(q => ({
              question: q.question,
              verification_type: q.verification_type || 'confidence_gap',
              linked_to: q.linked_to || '',
              reassuring_answer: q.reassuring_answer || '',
              concerning_answer: q.concerning_answer || '',
              confidence_level: q.confidence_level === 'high' ? 'high' : 'medium',
            }))
        }
      } catch (e) {
        console.warn('[score] Interview questions JSON parse failed, falling back to line parse:', e?.message)
      }
    }
    if (interviewQuestions.length === 0) {
      // Legacy fallback: numbered list produces plain strings. UI handles both.
      interviewQuestions = interviewRaw
...
Write the narrative now. Return prose only.`
  ...
  console.log('[score] Result assembled. Score:', result.overall_score, '| Risk:', result.risk_level, '| PassProb:', result.pass_probability, '| Trajectory:', result.trajectory, '| RedFlags:', result.red_flags.length, '| ConfGap:', result.confidence_competence_gap, '| FailurePatterns:', result.failure_patterns_detected.length)
  ...
  if (insertError) {
    ...
    if (/dimension_evidence|scoring_confidence|confidence_competence_gap|failure_patterns_detected/i.test(insertError.message || '')) {
      console.warn('[score] Retrying insert without dimension_evidence/scoring_confidence (migration not applied).')
