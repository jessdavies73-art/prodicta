...
## CANDIDATE TYPE
Write a plain-English label that describes this candidate the way a hiring manager would describe them to a colleague. Think: "You know that person who always... but tends to...". Write for a hiring manager who has never heard of PRODICTA, not for an assessment specialist.

Format exactly as one line:
CANDIDATE_TYPE: [plain description of how they work] with [their most important trait or risk]

Rules:
- Use plain conversational English only. No jargon, no compound nouns, no abstract labels.
- Good examples: "Reliable and Organised with a Tendency to Avoid Difficult Conversations", "Strong Communicator who Struggles Under Time Pressure", "Careful and Thorough but Slow to Make Decisions", "Confident and Direct but Misses Detail", "Hard Worker who Needs Clear Direction to Stay on Track".
- Bad examples (do not write like this): "Structured Process-Follower with Incomplete Quantitative Follow-Through", "Analytical Executor with Moderate Composure", "Tactical Operator with Deferred Accountability".
- A hiring manager should read it and immediately think "yes, that is exactly the person I just interviewed."
- 8-12 words total. No full stops. No hyphens joining compound words.
...
Write in plain UK English. No jargon. No bullet points. No bold text.`
  ...
  console.log('[score] CANDIDATE TYPE raw:', JSON.stringify(candidateTypeRaw.slice(0, 200)))
  ...
  console.log('[score] PREDICTED OUTCOMES raw:', JSON.stringify(predictionsRaw.slice(0, 300)))
  const predictionsLine = predictionsRaw.split('\n').find(l => /^PREDICTIONS:/i.test(l))
  console.log('[score] predictionsLine:', predictionsLine)
  if (predictionsLine) {
    const predsObj = {}
    predictionsLine.replace(/^PREDICTIONS:\s*/i, '').trim().split(/\s*\|\s*/).forEach(part => {
    ...
    console.log('[score] predsObj:', predsObj, '| keys:', Object.keys(predsObj).length)
