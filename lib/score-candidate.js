...
Write the narrative now. Return prose only.`
  ...
  if (insertError) {
    ...
    if (/dimension_evidence|scoring_confidence|confidence_competence_gap|failure_patterns_detected|consistency_summary|consistency_flag|percentile_basis|similar_candidate_pattern|generic_detection/i.test(insertError.message || '')) {
      console.warn('[score] Retrying insert without dimension_evidence/scoring_confidence/similar_candidate_pattern/generic_detection (migration not applied).')
