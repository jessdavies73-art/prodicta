...
export async function scoreCandidate(candidateId) {
  ...
  const { data: candidate, error: candError } = await adminClient
    .from('candidates')
    .select('*, assessments(role_title, job_description, scenarios, skill_weights, context_answers)')
    .eq('id', candidateId)
    .single()
...
${resp ? resp.response_text : '[No response provided]'}`
  ...
  const modeLabel = assessmentMode === 'quick' ? 'Quick Assessment' : assessmentMode === 'standard' ? 'Standard Assessment' : 'Advanced Assessment'
...
═══════════════════════════════════════════
This is a Quick Assessment, used for junior or urgent roles. Calibrate scoring as follows:
...
- DO NOT PENALISE for lack of strategic thinking, complex analysis, or commercial nuance. These are not senior roles. Marking down for missing strategy is a calibration error in Quick mode.
...
- TIMING EXPECTATIONS for Quick mode: each scenario should take 5 to 7 minutes. Under 2 minutes per scenario is suspicious. Over 10 minutes means the candidate took it seriously and should be viewed positively.`,

    standard: `═══════════════════════════════════════════
ASSESSMENT MODE: STANDARD (${scenarios.length} scenarios, ~25 to 30 minutes total)
═══════════════════════════════════════════
This is a Standard Assessment, used for most roles. Calibrate scoring as follows:
...
- TIMING EXPECTATIONS for Standard mode: 7 to 9 minutes per scenario. Under 3 minutes on any scenario is suspicious. If all 3 scenarios are completed in under 4 minutes each, flag this as an integrity concern.`,

    advanced: `═══════════════════════════════════════════
ASSESSMENT MODE: ADVANCED (${scenarios.length} scenarios, ~45 minutes total)
═══════════════════════════════════════════
This is an Advanced Assessment, used for senior roles. Calibrate scoring as follows:
...
- TIMING EXPECTATIONS for Advanced mode: 8 to 12 minutes per scenario. Under 3 minutes on any scenario is a red flag. Consistent 2-minute responses across all 4 scenarios is almost certainly AI-assisted.`,
  })[assessmentMode]
...
Assessment mode: ${modeLabel} (${scenarios.length} scenarios). When you write any integrity-related narrative or quality-of-response commentary, you must name the assessment mode and calibrate the language accordingly. For a Quick Assessment, brief responses are not by themselves a concern; for an Advanced Assessment, consistently short responses must be treated as a serious flag.
${trajectoryLine}
${patternLine}
...
Write in plain UK English. No jargon. No bullet points. No bold text.`
  ...
  const [narratives2aMsg, narratives2bMsg, narratives2cMsg] = await Promise.all([
...
Write the narrative now. Return prose only.`
    ...
    }).catch(err => { console.error('[score] Tuesday Reality error:', err?.message); return null })
    const tuesdayRaw = tuesdayMsg?.content?.[0]?.text || ''
    tuesdayReality = stripMd(tuesdayRaw) || null
    console.log('[score] Tuesday Reality generated in', Date.now() - tReadyStart, 'ms | length:', (tuesdayReality || '').length)
  } catch (trErr) {
    console.error('[score] Tuesday Reality failed (non-fatal):', trErr?.message)
  }
...
Create a coaching plan for the HIRING MANAGER to run across three phases. Tie every SMART objective, watch-out guide and prediction check back to the specific assessment data above, using the candidate's actual name.
