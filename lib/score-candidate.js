...
export async function scoreCandidate(candidateId) {
  ...
  const { data: candidate, error: candError } = await adminClient
    .from('candidates')
    .select('*, assessments(role_title, job_description, scenarios, skill_weights, context_answers, assessment_mode, role_level, detected_dimensions, dimension_rubrics, job_breakdown, employment_type, users(account_type))')
    .eq('id', candidateId)
    .single()
...
Write the narrative now. Return prose only.`
  ...
  if (insertError) {
    ...
    if (/dimension_evidence|scoring_confidence|confidence_competence_gap|failure_patterns_detected|consistency_summary|consistency_flag|percentile_basis|similar_candidate_pattern|generic_detection|ranking_quality|interruption_handling/i.test(insertError.message || '')) {
      console.warn('[score] Retrying insert without dimension_evidence/scoring_confidence/similar_candidate_pattern/generic_detection/ranking_quality/interruption_handling (migration not applied).')
