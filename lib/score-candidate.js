...
export async function scoreCandidate(candidateId) {
  ...
  // ── Markdown stripper — removes ** bold **, * italic *, ### headers ─────────
...
${resp ? resp.response_text : '[No response provided]'}`
  ...
  // ── Detect all-rushed: every scenario completed under 90 seconds ────────────
  ...
  // ── Word count per response (for short-response flagging) ──────────────────

  const wordCounts = scenarios.map((sc, i) => {
    const resp = responses.find(r => r.scenario_index === i)
    const wc = resp?.response_text?.trim().split(/\s+/).filter(Boolean).length ?? 0
    return `Scenario ${i + 1}: ${wc} words${wc < 50 ? ' [⚠ UNDER 50 WORDS — flag as insufficient engagement]' : ''}`
  }).join(' | ')
...
${scenarioSections}`
  ...
  const scoresPrompt = `You are a senior talent assessment specialist. A candidate has completed work simulation scenarios for the role of ${assessment.role_title}.

${context}

SCORING PHILOSOPHY — READ CAREFULLY:
Score harsh but fair. Use the FULL 0–100 range. Calibration:
- 0–39: Poor. Vague, generic, or disengaged responses with no role-specific thinking.
- 40–54: Below average. Some effort but significant gaps or shallow reasoning.
- 55–69: Average. Competent but unremarkable. What most candidates produce.
- 70–84: Good. Clear role-specific thinking, structured approach, genuine engagement.
- 85–100: Exceptional. Rare. Reserved for responses that would impress a hiring panel.
DO NOT cluster scores between 60–80. Differentiate genuinely. A candidate who gives vague, generic responses should score below 40. A candidate who demonstrates genuine role-specific thinking should score above 70.

STEP 1 — SKILL SCORES (0–100 each):
Skill weights: ${JSON.stringify(skillWeights)}
Score each skill based solely on the quality of reasoning, decisions, and actions demonstrated. Ignore spelling/grammar.

STEP 2 — PRESSURE-FIT SCORES (0–100 each, use full range):
- decision_speed_quality: Did they commit to clear decisions or hedge with "it depends"? Fast, confident decisions = high. Endless caveats = low.
- composure_under_conflict: Did they stay calm, professional, and constructive under pressure? Emotional language or avoidance = low.
- prioritisation_under_load: Did they use a clear framework (e.g. impact/urgency matrix) when demands competed? Reactive or ad-hoc = low.
- ownership_accountability: Did they use "I will" / "I would" language and own outcomes? Passive or blame-oriented language = low.

STEP 3 — SENIORITY FIT (0–100):
How well does the candidate's thinking match the seniority level of this role? Score 0 if they operate well below the expected level, 100 if they exceed it.

STEP 4 — RESPONSE QUALITY:
- "Genuine": Specific, personal, detailed responses that reference the scenario context
- "Likely Genuine": Mostly specific with some generic phrasing
- "Possibly AI-Assisted": Overly structured, formal language, lacks personal voice, suspiciously comprehensive
- "Suspicious": Identical structure across all responses, no personal voice, reads like a template

STEP 5 — COMPOSITE SCORING:
skills_weighted_avg = weighted average of skill scores using the weights provided
integrity_score = 100 if Genuine, 80 if Likely Genuine, 50 if Possibly AI-Assisted, 20 if Suspicious
...
confidence_level rules: "High" if responses are long and specific enough to assess reliably. "Medium" if some responses are short or vague. "Low" if responses are too brief to assess confidently (any under 50 words).
consistency_rating: "High" if quality is consistent across all scenarios. "Medium" if one scenario is notably weaker. "Low" if quality drops significantly in later scenarios (fatigue/disengagement).
...
percentile: "Top 10%", "Top 20%", "Top 30%", "Top 40%", "Top 50%", "Bottom 50%".`
  ...
  // ── Compute pass_probability from Call 1 scores ─────────────────────────────
  ...
  // Apply all-rushed penalty: -10 to overall_score, cap pass_probability at 60
  const rushedPenalty = allRushed ? 10 : 0
  const finalOverallScore = Math.max(0, scores.overall_score - rushedPenalty)
  const finalPassProbability = allRushed ? Math.min(passProbability, 60) : passProbability
  ...
  console.log('[score] pass_probability:', finalPassProbability, '| confidence_level:', scores.confidence_level, '| seniority_fit_score:', scores.seniority_fit_score, '| allRushed:', allRushed)
...
CRITICAL FORMATTING RULE: Do not use any markdown formatting whatsoever. No **bold**, no *italic*, no ### headers, no bullet points with *, no backticks. Write in plain prose only. The system will handle all formatting — markdown syntax will appear as raw characters to the reader.

Write the following sections. Use the exact headers shown (## HEADER only for section boundaries — nowhere else). Every section must quote or directly reference specific things this candidate wrote.
...
## ONBOARDING PLAN
Write exactly 6 entries — Week 1 through Week 6 ONLY. Do not write Week 7 or beyond. Each week must be on its own line starting with exactly "Week N:" (e.g. Week 1:, Week 2:). Each entry must be specific to this candidate's gaps — not generic onboarding advice. Reference UK employment practices where relevant (e.g. CIPD probation guidance, ERA 2025 day-one rights). Minimum 2 sentences per week. Stop after Week 6.
...
Two sentences. First: describe the consistency pattern across responses. Second: what this suggests about the candidate under varying conditions.`
  ...
  console.log('[score] Result assembled. Score:', result.overall_score, '| Risk:', result.risk_level, '| PassProb:', result.pass_probability, '| RedFlags:', result.red_flags.length)
