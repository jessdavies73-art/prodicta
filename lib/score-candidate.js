...
Write the narrative now. Return prose only.`
  ...
  // Stash predictions_verification on the predictions JSONB itself so we do not
  // need a new DB column. Readers pull predictions._verification when rendering
  // the inline "Ask at interview" section under each predicted outcome.
  const predictionsForResult = predictionsVerification && predictions
    ? { ...predictions, _verification: predictionsVerification }
    : predictions
