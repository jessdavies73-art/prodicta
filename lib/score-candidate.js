...

{
  ...
  function repairJson(raw) {
    ...
    // Walk char-by-char to only fix newlines that are inside strings
    let out = ''
    let inStr = false
    let escaped = false
  ...
  try {
    message = await anthropic.messages.create({
      ...
      max_tokens: 5000,
      system: 'You are a JSON API. Return only valid JSON. In all string values: escape any double quotes as \\", replace newlines with \\n. Keep all narrative fields under 150 words. No markdown, no code fences, no text outside the JSON object.',
  ...
  let result
  // Attempt 1: repaired JSON
  try {
    result = JSON.parse(repaired)
    console.log('[score] JSON parsed successfully on attempt 1.')
  } catch (parseErr1) {
    console.error('[score] Attempt 1 parse failed:', parseErr1?.message)
    console.error('[score] Raw preview (500):', content.slice(0, 500))
    console.error('[score] Repaired preview (500):', repaired.slice(0, 500))

    // Attempt 2: ask Claude to return scores only, no narratives
    console.log('[score] Falling back to scores-only API call...')
    try {
      const fallbackMsg = await anthropic.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 800,
        system: 'You are a JSON API. Return only valid JSON, no markdown, no text outside the object.',
        messages: [{
          role: 'user',
          content: `Based on the candidate assessment you just scored, return ONLY this JSON with no narrative text (scores only):
{
  "overall_score": <number>,
  "scores": { ${skillNames.map(s => `"${s}": <number>`).join(', ')} },
  "risk_level": "<Very Low|Low|Medium|High>",
  "risk_reason": "<one short sentence>",
  "percentile": "<Top X%>",
  "ai_summary": "<two sentences max>",
  "strengths": [{"text": "<short>", "evidence": "<short>"}],
  "watchouts": [],
  "score_narratives": { ${skillNames.map(s => `"${s}": "<one sentence>"`).join(', ')} },
  "onboarding_plan": ["Week 1: Review performance expectations"],
  "interview_questions": ["Tell me about a time you handled a difficult client."],
  "percentile": "Top 30%",
  "pressure_fit_score": 70,
  "pressure_fit": {},
  "response_quality": "Genuine",
  "quality_notes": "Responses appear authentic.",
  "time_analysis": "Normal timing across scenarios.",
  "red_flags": [],
  "consistency_rating": "High",
  "consistency_notes": "Consistent approach throughout."
}
Original candidate responses were already assessed. Use your previous scoring judgement for the numbers.`
        }],
      })
      const fallbackContent = fallbackMsg.content[0].text.trim()
      const fallbackRepaired = repairJson(fallbackContent)
      result = JSON.parse(fallbackRepaired)
      console.log('[score] Fallback parse succeeded. Score:', result.overall_score)
    } catch (fallbackErr) {
      console.error('[score] Fallback parse also failed:', fallbackErr?.message)
      throw new Error('JSON parsing failed on both attempts: ' + parseErr1.message)
    }
  }
  console.log('[score] Result parsed. Score:', result.overall_score, '| Risk:', result.risk_level)
