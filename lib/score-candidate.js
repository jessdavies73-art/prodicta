...
import { sendScoringCompleteAlert } from '@/lib/send-scoring-complete-alert'
...
// ─────────────────────────────────────────────────────────────────────────
// Function-keyed Pressure-Fit dimension cues. Mirrors the
// FUNCTION_SCENARIO_HINTS pattern in lib/scenario-generator.js. The four
// PF dimensions and the scoring math are unchanged; only the cue text
// the model is told to look for changes per function.
//
// Healthcare and education functions get role-native vocabulary
// (handover, MAR chart, DSL, CPOMS, behaviour log, parent at the gate).
// Office functions inherit the existing office-coded cues via the
// 'office_default' fallback.
//
// Lookup is keyed on role_profile.function (set by
// lib/role-profile-detector.js). Legacy assessments without role_profile
// fall through to office_default.
// ─────────────────────────────────────────────────────────────────────────
export const PF_DIMENSION_HINTS_BY_FUNCTION = {
  healthcare_clinical: {
    decision_speed_quality:
      'Did the candidate make a clinical or care decision quickly when the situation required it (escalating to the senior nurse or doctor, deciding to call the on-call SHO, choosing to involve the family)? Indicators show a clear next step with a named recipient and timing, not "I would probably observe" or "I think I might".',
    composure_under_conflict:
      'How did the candidate handle the distressed patient or family member, the colleague disagreement on a care plan, or the difficult phone call from a relative? Evidence suggests professional composure when they acknowledge emotion or concern before pivoting to the clinical or factual answer.',
    prioritisation_under_load:
      'When competing demands stacked up (medication round, deteriorating patient, family at the bedside, handover documentation, safeguarding concern), did the candidate sequence by clinical urgency and patient safety? Indicators show they named the rationale (e.g. "Bed 4 first because NEWS2 is rising") rather than firefighting whoever shouted loudest.',
    ownership_accountability:
      'Did the candidate take responsibility for the artefacts that matter on shift (handover note, MAR chart entry, incident form, safeguarding referral, escalation to the doctor)? Indicators show "I will document", "I will hand over", "I will complete the form" rather than "the team should" or "documentation needs to be done".',
  },
  healthcare_care: {
    decision_speed_quality:
      'Did the candidate make a care decision quickly when the situation required it (escalating to the senior carer or nurse, deciding to call the GP or family, choosing to involve the registered manager)? Indicators show a clear next step with a named recipient and timing, not vague qualifiers.',
    composure_under_conflict:
      'How did the candidate handle the distressed resident, the agitated family member, or the colleague disagreement? Evidence suggests professional composure when they acknowledge emotion before pivoting to the practical care or boundary response, and stay within scope of practice rather than improvising a clinical decision.',
    prioritisation_under_load:
      'When competing care demands stacked up (personal care round, residents calling out, family visit, safeguarding concern, handover documentation), did the candidate sequence by resident safety and dignity? Indicators show they named the rationale and knew when to escalate to the senior carer or nurse, rather than defaulting to whoever shouted loudest.',
    ownership_accountability:
      'Did the candidate take responsibility for the records that matter on shift (handover note, daily care record, body map, incident form, safeguarding alert, MAR chart entry within scope)? Indicators show "I will record this", "I will hand over", "I will tell the senior carer" rather than "someone should" or "the team needs to".',
  },
  social_work: {
    decision_speed_quality:
      'Did the candidate make a casework decision quickly when statutory or safeguarding pressure required it (initiating Section 47, contacting MASH, contacting the police, escalating to the team manager)? Indicators show a clear next step grounded in threshold language, not vague qualifiers.',
    composure_under_conflict:
      'How did the candidate handle the angry parent, the carer in crisis, or the colleague disagreement on threshold? Evidence suggests professional composure when they acknowledge emotion or position before holding the safeguarding line, and stay defensible in record-keeping language.',
    prioritisation_under_load:
      'When caseload demands competed (s47 visit, statutory review, court report, multi-agency meeting, urgent safeguarding referral), did the candidate sequence by statutory deadline and risk to the person at the centre? Indicators show they named the rationale and recognised when to escalate to the team manager.',
    ownership_accountability:
      'Did the candidate take responsibility for the casework artefacts that matter (case note, chronology entry, statutory letter, court bundle entry, MASH referral)? Indicators show "I will record", "I will refer", "I will brief the manager" rather than abstract description of what should happen.',
  },
  education_teaching: {
    decision_speed_quality:
      'Did the candidate act quickly when professional judgement required it (behaviour incident in lesson, safeguarding flag in a piece of work, parent at the classroom door, pupil disclosure)? Patterns suggest a clear next step with a named adult or system, not "I would probably mention it" or "I think I might check".',
    composure_under_conflict:
      'How did the candidate handle the angry parent, the disruptive pupil, or the colleague disagreement on a sanction? Professional judgement appeared when they acknowledged the feeling or position before holding the boundary, and stayed within the school behaviour or safeguarding policy rather than improvising.',
    prioritisation_under_load:
      'When competing demands stacked up (lesson preparation, behaviour incident, parent communication, marking, safeguarding concern, SENCO query), did the candidate sequence by pupil safety and learning impact? Patterns suggest they named the rationale and knew when to escalate to the year leader, SENCO, or DSL.',
    ownership_accountability:
      'Did the candidate take responsibility for the artefacts that matter (behaviour log entry, CPOMS / MyConcern record, parent contact log, SEN concern flag to the SENCO, DSL referral where relevant)? Patterns suggest "I will log", "I will refer", "I will phone the parent" rather than "the school should" or "someone needs to".',
  },
  education_support: {
    decision_speed_quality:
      'Did the candidate act quickly when professional judgement required it within their scope (escalating a behaviour issue to the teacher, flagging a pupil disclosure, raising a safeguarding concern)? Patterns suggest a clear next step with a named adult, not vague qualifiers.',
    composure_under_conflict:
      'How did the candidate handle the upset pupil, the parent at the gate, or the colleague disagreement? Professional judgement appeared when they held a calm, in-scope boundary while acknowledging the feeling, and escalated rather than improvising outside their role.',
    prioritisation_under_load:
      'When competing demands stacked up (1-to-1 support pupil, small group intervention, behaviour incident, safeguarding flag, lesson resource prep), did the candidate sequence by pupil safety and the teacher\'s plan? Patterns suggest they recognised when to involve the class teacher, SENCO, or DSL.',
    ownership_accountability:
      'Did the candidate take responsibility for the records they own (handover to the teacher, CPOMS / MyConcern entry within scope, intervention log)? Patterns suggest "I will tell the class teacher", "I will record this", "I will pass it to the DSL" rather than abstract description.',
  },
  office_default: {
    decision_speed_quality:
      'Did they commit to decisions or hedge? Look for clear next steps with timelines vs vague qualifiers ("I would probably", "I think I\'d"). Did they name what they would do first and why, or stay in analysis mode?',
    composure_under_conflict:
      'How did they handle the difficult person in the scenario? Did they acknowledge feelings before problem-solving? Did they stay firm or cave under pressure? Did they propose a specific resolution or defer?',
    prioritisation_under_load:
      'What framework did they use? Did they explain WHY they ordered things that way, or just list them? Did they identify dependencies? Did they consider delegating anything, or try to own everything themselves?',
    ownership_accountability:
      'Count the language patterns. "I will" and "I would" vs "someone should" and "the team needs to". Did they commit to specific actions with deadlines, or describe what ought to happen in the abstract?',
  },
}

// Resolve PF dimension cues by role_profile.function. Returns the
// office_default entry for any function not in the table or for legacy
// assessments where role_profile is null.
export function pfDimensionHintsFor(role_profile) {
  const fn = role_profile?.function || null
  if (fn && PF_DIMENSION_HINTS_BY_FUNCTION[fn]) return PF_DIMENSION_HINTS_BY_FUNCTION[fn]
  return PF_DIMENSION_HINTS_BY_FUNCTION.office_default
}

// ─────────────────────────────────────────────────────────────────────────
// Shell-keyed Monday Morning Reality framing. Replaces the hard-coded
// office-cliché opener ("Monday morning when everything lands at once"
// and "which meetings or tasks will go well") with shell-native framing.
// Office shell keeps the original wording. Legacy assessments without
// shell_family fall through to office.
// ─────────────────────────────────────────────────────────────────────────
export const MMR_FRAMING_BY_SHELL = {
  office: {
    moment: 'Monday morning when everything lands at once',
    surface: 'which meetings or tasks will go well',
  },
  healthcare: {
    moment: 'the start of a busy shift when handover, family visits, and care priorities all converge',
    surface: 'which clinical decisions, family conversations, or care plans will land well, and which carry risk',
  },
  education: {
    moment: 'the start of a typical school day when lessons, parent communication, behaviour incidents, and pupil concerns all converge',
    surface: 'which lessons, parent communications, or interventions will land well, and which carry risk',
  },
}

export function mmrFramingFor(shell_family) {
  return MMR_FRAMING_BY_SHELL[shell_family] || MMR_FRAMING_BY_SHELL.office
}

// ─────────────────────────────────────────────────────────────────────────
// Shell-keyed interview question vocabulary cues. Injected into the
// verification prompt that produces interview_questions on the result.
// The four prediction questions (pass_probation / top_performer /
// churn_risk / underperformance_risk) are always generated regardless
// of the candidate's watch-out density, so without this injection
// healthcare and education candidates get office-coded interview
// questions ("manage competing stakeholder priorities") even when the
// watch-out anchors are shell-correct.
//
// Each shell entry lists 5 example questions in role-native vocabulary.
// The model uses these as a vocabulary palette, not a strict template;
// the watch-out and prediction anchors still drive question selection.
//
// Office shell preserves the current generic-stakeholder-management
// framing. Legacy assessments without shell_family fall through to
// office.
// ─────────────────────────────────────────────────────────────────────────
export const INTERVIEW_QUESTION_HINTS_BY_SHELL = {
  office: `Office-shell interview vocabulary palette. Use any of these question patterns where they fit the watch-out or prediction anchor:
- Stakeholder management: "Tell me about a time you managed competing stakeholder priorities. Walk me through who pulled in which direction and what you actually decided."
- Project complexity: "Describe a project where the brief shifted partway through. What was your read on the change, what did you protect, and what did you concede?"
- Decision under uncertainty: "Tell me about a decision you owned where the data was incomplete. How did you frame the call and what did you do if you got it wrong?"
- Ownership: "Describe a piece of work where you were on the hook end-to-end. What did you escalate, what did you carry yourself, and how did you keep your manager informed?"
- Accountability under pressure: "Walk me through a deadline week when something went wrong. What did you tell who, when, and how did you manage the fall-out?"`,

  healthcare: `Healthcare-shell interview vocabulary palette. Use any of these question patterns where they fit the watch-out or prediction anchor. Use clinical and care vocabulary, not generic stakeholder language:
- Handover and documentation: "Tell me about a difficult handover and how you protected resident or patient continuity. What went into your handover note, what did you escalate verbally, and what would you do differently?"
- Family / relative communication: "How would you handle a family member who is distressed about their relative's care? Walk me through how you would acknowledge the concern while still holding the safeguarding or clinical line."
- Safeguarding judgement: "Describe a time you flagged a safeguarding concern. What did you escalate and to whom, and how did you record it? Indicators show a strong answer names the named recipient and the system used."
- Scope of practice: "Tell me about a time you held the line on scope when pressure was mounting. What did you decline, what did you escalate, and how did you communicate the boundary?"
- Records and incident reporting: "What goes in a daily care record vs an incident form vs a body map? Walk me through how you would document yesterday's hypothetical incident, including who you would copy and what timing you would use."
- Compliance language for the response: "indicators show", "evidence suggests", "clinical judgement appeared sound". Avoid definitive claims about clinical competence.`,

  education: `Education-shell interview vocabulary palette. Use any of these question patterns where they fit the watch-out or prediction anchor. Use teaching, pastoral and safeguarding vocabulary, not generic stakeholder language:
- Behaviour management: "Tell me about a difficult behaviour incident and how you de-escalated without breaking the relationship with the pupil. What did you say in the first 30 seconds and what did you log afterwards?"
- Parent communication: "How would you handle a parent who is angry about a marking decision or a sanction? Patterns suggest a strong answer acknowledges the feeling before holding the boundary."
- Safeguarding judgement: "Describe a time you flagged a safeguarding concern. What did you escalate to the DSL, how did you record it on CPOMS or MyConcern, and what was the threshold call you weighed?"
- Curriculum and differentiation: "Walk me through how you would plan a lesson for a mixed-attainment class with two pupils on EHCPs. What goes in your support plan, what scaffolding do you put in for the lower attainers, and what extension for the higher attainers?"
- Records and pathway: "What goes in CPOMS vs a behaviour log vs a parent contact log? When would you use each, and what is the threshold for a DSL referral vs a behaviour entry?"
- Compliance language for the response: "patterns suggest", "professional judgement appeared", "the response reads as appropriate to scope". Avoid definitive claims about teaching competence or safeguarding outcomes.`,
}

export function interviewHintsFor(shell_family) {
  return INTERVIEW_QUESTION_HINTS_BY_SHELL[shell_family] || INTERVIEW_QUESTION_HINTS_BY_SHELL.office
}

// ─────────────────────────────────────────────────────────────────────────
// Shell-keyed onboarding plan vocabulary cues. Layers on top of the
// existing role_level branch (OPERATIONAL / LEADERSHIP / mid). The
// role_level branch sets the altitude (day-one ops vs strategic listening
// tour vs 30-60-90 quick wins); this lookup sets the role-native
// vocabulary the activities should use.
//
// Without this cue, healthcare and education candidates' onboarding plans
// land office-default phrases ("Week 1: Map the stakeholder landscape",
// "deliver first visible quick-win project by week 3") that read as
// obviously wrong to a care home manager or a head teacher.
//
// Office shell preserves the current behaviour. Legacy assessments
// without shell_family fall through to office.
// ─────────────────────────────────────────────────────────────────────────
export const ONBOARDING_PLAN_HINTS_BY_SHELL = {
  office: `Office-shell onboarding vocabulary. Activities should reference: stakeholder mapping, 1:1s with line manager and direct reports, the rhythm of the team meetings, the team's tooling (CRM, project board, OKR tracker), the first quick-win project, the manager's success criteria, the probation review.`,

  healthcare: `Healthcare-shell onboarding vocabulary. Activities must reference role-native artefacts and routines, not office tropes. Across the 6 weeks the plan should cover:
- Week 1: shadow a senior carer or registered nurse through one full medication round and one full handover; complete mandatory induction (manual handling, fire safety, infection control, mandatory safeguarding); meet residents or patients on the unit and learn their care plans; meet families where present.
- Weeks 2 to 4: buddy with experienced colleagues; observe complex care interactions (challenging behaviour, end-of-life, family conflict, safeguarding-adjacent); complete medication competency sign-off where in scope; first independent handover with senior review.
- Weeks 5 to 8: take own assignment with senior support on shift; complete monthly supervision; demonstrate documentation discipline (daily care record, incident form, body map, MAR chart entry within scope); first independent family conversation with senior review.
- Weeks 9 to 12: confidence in autonomous practice within scope; complete probation review; CQC-aligned competency check.
- Use clinical and care vocabulary throughout: handover, MAR chart, daily care record, incident form, body map, safeguarding referral, senior carer, registered manager, escalation to senior nurse. Avoid "stakeholder", "deliverables", "OKRs", "quick-win project". Compliance language: "indicators show", "evidence suggests".
- Success criteria should reference: care competencies, escalation judgement, documentation quality, family communication, dignity and respect, scope of practice.`,

  education: `Education-shell onboarding vocabulary. Activities must reference role-native artefacts and routines, not office tropes. Across the 6 weeks the plan should cover:
- Weeks 1 to 2: observe planning and preparation time with the year-group lead; meet the year team and key colleagues (SENCO, DSL, pastoral lead, deputy head); walk through the school behaviour policy and the safeguarding pathway; meet pupils on register and review their EHCPs and SEN flags; observe a classroom of the same year group.
- Weeks 3 to 4: begin teaching with mentoring from the year-group lead; observe a complex parent conversation; complete safeguarding induction (KCSIE Part 1, school-specific safeguarding pathway, CPOMS or MyConcern training); first behaviour log entry signed off by the year leader.
- Weeks 5 to 8: take ownership of own classes with reduced mentoring; embed assessment for learning routines; regular line management meetings; first independent parent communication signed off by the year leader; SENCO check-in on EHCP pupils.
- Weeks 9 to 12: independent practice confidence; probation review against the Teachers' Standards (DfE); evidence file of behaviour logs and parent contacts.
- Use teaching, pastoral and safeguarding vocabulary throughout: lesson, year team, year-group lead, SENCO, DSL, CPOMS, MyConcern, behaviour log, parent contact log, EHCP, SEN, KCSIE, Teachers' Standards. Avoid "stakeholder", "deliverables", "OKRs", "quick-win project". Compliance language: "patterns suggest", "professional judgement appeared".
- Success criteria should reference: teaching standards, behaviour management, safeguarding judgement, parent engagement, professional development, evidence of impact on pupil progress.`,
}

export function onboardingHintsFor(shell_family) {
  return ONBOARDING_PLAN_HINTS_BY_SHELL[shell_family] || ONBOARDING_PLAN_HINTS_BY_SHELL.office
}
...
export async function scoreCandidate(candidateId) {
  ...
  const { data: candidate, error: candError } = await adminClient
    .from('candidates')
    .select('*, assessments(role_title, job_description, scenarios, skill_weights, context_answers, assessment_mode, role_level, detected_dimensions, dimension_rubrics, job_breakdown, employment_type, role_profile, workspace_scenario, use_modular_workspace, shell_family, strategy_fit_components, strategic_thinking_responses, users(account_type))')
    .eq('id', candidateId)
    .single()
...
─────────────────────────────────────────────
HEALTHCARE / SOCIAL CARE FAILURE PATTERNS , SCORE THESE EXPLICITLY
─────────────────────────────────────────────

These shell-specific patterns apply IN ADDITION to the universal seven above. Compliance language: indicators show, evidence suggests, patterns suggest. No definitive clinical claims.

FAILURE PATTERN 8 , MISSED HANDOVER DETAIL
Signals: Candidate gives or receives a handover and does not capture key clinical or care information. Glosses over a deteriorating observation, an unresolved family concern, an outstanding referral, or a medication change. Treats handover as a formality.
Impact: Lower communication and execution_reliability scores. Generate watch-out: "Did not capture key clinical or care information at handover."
Distinction: Brief handover is fine when nothing is outstanding. Flag only when the scenario contains specific information the candidate should have captured and they did not.

FAILURE PATTERN 9 , BYPASSED SENIOR ON CLINICAL ESCALATION
Signals: Candidate makes an autonomous decision in a situation that clearly required senior input (senior nurse, doctor, ward manager, registered manager). Indicators show the candidate proceeded without seeking the level of authority the situation called for.
Impact: Lower escalation and ownership scores. Generate watch-out: "Did not escalate when appropriate to senior nurse, doctor, or manager."
Distinction: Confident handling within scope is good. Flag only when the scenario indicators show the situation exceeded the candidate's autonomous scope.

FAILURE PATTERN 10 , DOCUMENTATION GAP
Signals: Candidate completes a task (observation, medication round, incident response, family interaction) but does not document it appropriately. Patterns suggest reliance on memory or verbal handover where written record was required.
Impact: Lower documentation and compliance scores. Generate watch-out: "Did not document observation, medication, or incident appropriately."
Distinction: Quick verbal updates are fine alongside documentation, not instead of it. Flag only when the scenario required a written record and the candidate did not produce one.

FAILURE PATTERN 11 , MISSED SAFEGUARDING REFERRAL
Signals: Candidate observes a safeguarding indicator (vulnerability, harm, neglect, capacity concern, family dynamic) and does not flag or escalate it. Treats it as a clinical or operational issue without recognising the safeguarding dimension.
Impact: Lower safeguarding awareness and compliance scores. Generate watch-out: "Did not flag or escalate safeguarding concern."
Distinction: Not every concern is a safeguarding referral. Flag only when the scenario indicators clearly meet the threshold and the candidate did not recognise it.

FAILURE PATTERN 12 , WORKED THROUGH ROUND WITHOUT CHECKS
Signals: Candidate completes a round of observations, medications, or care tasks without checking on patient or resident status. Evidence suggests task completion took precedence over the people the task was for.
Impact: Lower safety and empathy scores. Generate watch-out: "Skipped patient/resident checks while completing other tasks."
Distinction: Efficient task completion is good. Flag only when the scenario contained signals (a deteriorating patient, a distressed resident, a fall risk) the candidate should have noticed.

FAILURE PATTERN 13 , FAMILY OR VISITOR CONFRONTATION ESCALATED
Signals: A family member, visitor, or relative arrives upset or hostile. The candidate does not de-escalate and the exchange escalates further. Patterns suggest the candidate matched intensity rather than reducing it.
Impact: Lower communication and composure scores. Generate watch-out: "Allowed family or visitor exchange to escalate without de-escalation."
Distinction: Setting boundaries firmly is fine. Flag only when the scenario indicators show de-escalation was the appropriate response and the candidate did not attempt it.

FAILURE PATTERN 14 , SOLE WORKING WITHOUT BACKUP
Signals: Candidate is working alone (lone worker, late shift, agency cover) and the scenario complexity exceeds safe lone working. Evidence suggests the candidate continued without calling for support.
Impact: Lower safety and judgement scores. Generate watch-out: "Did not call for support when shift complexity required it."
Distinction: Independent working within capacity is fine. Flag only when the scenario indicators clearly exceeded what should be handled alone.
` : ''}${assessment.shell_family === 'education' ? `
─────────────────────────────────────────────
EDUCATION FAILURE PATTERNS , SCORE THESE EXPLICITLY
─────────────────────────────────────────────

These shell-specific patterns apply IN ADDITION to the universal seven above. Compliance language: patterns suggest, professional judgement appeared, evidence suggests. Anonymise pupils. Never name a real school.

FAILURE PATTERN 8 , DID NOT LOG BEHAVIOUR INCIDENT
Signals: A pupil behaviour incident occurs and the candidate does not record it on the school's behaviour management system or the appropriate alternative. Patterns suggest the incident was treated verbally only.
Impact: Lower documentation and compliance scores. Generate watch-out: "Skipped or delayed entry to behaviour management system."
Distinction: Brief verbal acknowledgement at the time is fine alongside the system entry. Flag only when the scenario required a logged record and the candidate did not produce one.

FAILURE PATTERN 9 , MISSED SAFEGUARDING REFERRAL TO DSL
Signals: A pupil discloses, indicators show neglect, or behaviour suggests a wider safeguarding concern. The candidate does not refer to the Designated Safeguarding Lead or appropriate authority.
Impact: Lower safeguarding and compliance scores. Generate watch-out: "Did not escalate safeguarding concern to DSL or appropriate authority."
Distinction: Not every concern is a safeguarding referral. Flag only when the scenario indicators clearly meet the threshold and the candidate did not refer.

FAILURE PATTERN 10 , PARENT CONFRONTATION ESCALATED
Signals: A parent communication (email, phone call, at the school office) starts negative and escalates further during the candidate's response. Professional judgement appeared not to include de-escalation moves (acknowledgement, structure, named next step).
Impact: Lower communication and composure scores. Generate watch-out: "Allowed parent communication to escalate to formal complaint without intervention."
Distinction: Holding a position the parent disagrees with is fine. Flag only when the scenario indicators show de-escalation was available and the candidate did not attempt it.

FAILURE PATTERN 11 , DID NOT FLAG SEN CONCERN
Signals: Candidate notices a pupil pattern (Pupil A, Pupil B) that suggests SEN need, EAL difficulty, or specialist input requirement, and does not refer to the SENCO. Patterns suggest the concern was treated as ordinary difficulty.
Impact: Lower stakeholder awareness and compliance scores. Generate watch-out: "Did not refer to SENCO when pupil need patterns suggested specialist input."
Distinction: General classroom adaptation is fine. Flag only when the scenario indicators clearly suggested specialist referral was warranted.

FAILURE PATTERN 12 , SKIPPED DSL INVOLVEMENT ON DISCLOSURE
Signals: A pupil disclosure occurs (about home life, peer harm, online concern, self-harm) and the candidate handles it pastorally without involving the Designated Safeguarding Lead.
Impact: Lower safeguarding and compliance scores significantly. Generate watch-out: "Did not refer to Designated Safeguarding Lead when disclosure required it."
Distinction: Pastoral support around the disclosure is fine. Flag only when the scenario indicators required the DSL pathway and the candidate did not use it.

FAILURE PATTERN 13 , LESSON PREPARATION GAP
Signals: Candidate plans or delivers a lesson without adequate preparation, materials, or differentiation for SEN, EAL, HAP, or LAP pupils. Evidence suggests the lesson assumed a single ability profile.
Impact: Lower preparation and inclusion scores. Generate watch-out: "Did not adequately prepare materials or differentiation for SEN/EAL/HAP/LAP."
Distinction: Time-pressured lessons happen. Flag only when the scenario indicators show the candidate had time to prepare differentiation and chose not to.

FAILURE PATTERN 14 , PUPIL PROGRESS CONCERN UNADDRESSED
Signals: Candidate notices a pupil progress pattern (sudden drop, persistent gap, attendance change) and does not flag it for intervention. Patterns suggest the concern was logged and forgotten.
Impact: Lower follow-through and stakeholder scores. Generate watch-out: "Did not flag pupil pattern that warranted intervention."
Distinction: Routine fluctuation is normal. Flag only when the scenario indicators show a clear pattern that required action.
` : ''}
DETECTION INSTRUCTION:
After reading each scenario response, run through every applicable pattern${assessment.shell_family === 'healthcare' ? ' (universal 1 to 7 plus healthcare 8 to 14)' : assessment.shell_family === 'education' ? ' (universal 1 to 7 plus education 8 to 14)' : ' (universal 1 to 7)'}. Note which ones are present. If two or more patterns appear in the same response, flag the most significant one as a primary watch-out and note the second as a contributing factor in the narrative.
...
"Hesitation under pressure", "Works in isolation", "Defaults to escalation", "Overcomplicates under pressure", "Rushes without considering consequences", "Avoids difficult conversations", "Externalises blame"${assessment.shell_family === 'healthcare' ? `, "Missed handover detail", "Bypassed senior on clinical escalation", "Documentation gap", "Missed safeguarding referral", "Worked through round without checks", "Family or visitor confrontation escalated", "Sole working without backup"` : ''}${assessment.shell_family === 'education' ? `, "Did not log behaviour incident", "Missed safeguarding referral to DSL", "Parent confrontation escalated", "Did not flag SEN concern", "Skipped DSL involvement on disclosure", "Lesson preparation gap", "Pupil progress concern unaddressed"` : ''}.
...
Write the sections below using the exact ## HEADER markers shown. Every section must quote or directly reference specific things this candidate actually wrote.`
  ...
  //
  // pfHints injects function-keyed Pressure-Fit dimension cues into the
  // PRESSURE FIT NARRATIVES section. Healthcare and education roles get
  // role-native vocabulary (handover, MAR chart, DSL, CPOMS); office
  // roles get the unchanged office-coded cues via office_default
  // fallback. Legacy assessments without role_profile fall through.
  const pfHints = pfDimensionHintsFor(assessment?.role_profile)
...
${assessment.shell_family === 'healthcare' ? `
Shell-specific (healthcare / social care) watch-out titles:
- "Missed handover detail" -> watch-out title: "Did not capture key clinical or care information at handover"
- "Bypassed senior on clinical escalation" -> watch-out title: "Did not escalate when appropriate to senior nurse, doctor, or manager"
- "Documentation gap" -> watch-out title: "Did not document observation, medication, or incident appropriately"
- "Missed safeguarding referral" -> watch-out title: "Did not flag or escalate safeguarding concern"
- "Worked through round without checks" -> watch-out title: "Skipped patient/resident checks while completing other tasks"
- "Family or visitor confrontation escalated" -> watch-out title: "Allowed family or visitor exchange to escalate without de-escalation"
- "Sole working without backup" -> watch-out title: "Did not call for support when shift complexity required it"

Compliance language for healthcare watch-outs: indicators show, evidence suggests, patterns suggest. Never make definitive clinical claims. Use placeholder language for clinical detail (the prescribed analgesia, the regular round). No real drug names. No specific clinical advice.
` : ''}${assessment.shell_family === 'education' ? `
Shell-specific (education) watch-out titles:
- "Did not log behaviour incident" -> watch-out title: "Skipped or delayed entry to behaviour management system"
- "Missed safeguarding referral to DSL" -> watch-out title: "Did not escalate safeguarding concern to DSL or appropriate authority"
- "Parent confrontation escalated" -> watch-out title: "Allowed parent communication to escalate to formal complaint without intervention"
- "Did not flag SEN concern" -> watch-out title: "Did not refer to SENCO when pupil need patterns suggested specialist input"
- "Skipped DSL involvement on disclosure" -> watch-out title: "Did not refer to Designated Safeguarding Lead when disclosure required it"
- "Lesson preparation gap" -> watch-out title: "Did not adequately prepare materials or differentiation for SEN/EAL/HAP/LAP"
- "Pupil progress concern unaddressed" -> watch-out title: "Did not flag pupil pattern that warranted intervention"

Compliance language for education watch-outs: patterns suggest, professional judgement appeared, evidence suggests. Anonymise pupils (Pupil A, Pupil B). Never name a real school. Reference Ofsted, EHCP, Pupil Premium by name only when the role makes them relevant.
` : ''}
...
Dimension-specific focus (cues are framed for this role's function; use the role-native vocabulary, not generic office language):
DECISION_SPEED_QUALITY: ${pfHints.decision_speed_quality}
COMPOSURE_UNDER_CONFLICT: ${pfHints.composure_under_conflict}
PRIORITISATION_UNDER_LOAD: ${pfHints.prioritisation_under_load}
OWNERSHIP_ACCOUNTABILITY: ${pfHints.ownership_accountability}
...
CULTURE_FIT_POINT: [ALIGNMENT or FRICTION] | [one sentence, optional]`
  ...
  // Two-axis branching: roleLevel sets the altitude (day-one ops vs
  // strategic listening tour vs 30-60-90 quick wins); shell_family sets
  // the role-native vocabulary the activities should use. Healthcare
  // and education shells get sector-appropriate artefacts (handover,
  // MAR chart, CPOMS, DSL, year team) layered on top of the altitude
  // guidance. Office shell preserves the original behaviour. Legacy
  // assessments without shell_family fall through to office.
  const onboardingShellHints = onboardingHintsFor(assessment?.shell_family)
  const onboardingGuidance = (roleLevel === 'OPERATIONAL'
    ...
    + `\n\nShell-specific vocabulary cue. Use the role-native artefacts and routines below for every activity. The altitude guidance above sets the level; this cue sets the language:\n${onboardingShellHints}`
...
{"workspace_score": 0-100, "workspace_narrative": "3-4 sentences", "workspace_signals": ["string"], "workspace_watch_out": "string or null"}` }],
  ...
  // Strategy-Fit Strategic Thinking Evaluation scoring. Runs only when:
  //   - mode === 'advanced' (Strategy-Fit), AND
  //   - the component was generated at create time (lives on
  //     assessments.strategy_fit_components.strategic_thinking with at
  //     least one evaluation_question), AND
  //   - the candidate's responses landed on the assessment row via the
  //     submit route (lives on assessments.strategic_thinking_responses
  //     with at least one populated answer).
  // Result is appended to the workspaceBlockScores array as a synthetic
  // block keyed 'strategic-thinking' so the existing report drill-down,
  // PDF render path, Highlight Reel slide builder, and Executive Summary
  // synthesis pick it up automatically. Failure is non-blocking: if the
  // scorer returns null, the report renders the rest of the assessment
  // unchanged and the Strategic Thinking section is silently absent.
  if (assessment?.assessment_mode === 'advanced') {
    try {
      const stComponent = assessment?.strategy_fit_components?.strategic_thinking
      const stResponses = assessment?.strategic_thinking_responses
      const stHasComponent = stComponent
        && Array.isArray(stComponent.evaluation_questions)
        && stComponent.evaluation_questions.length > 0
      const stHasResponses = stResponses
        && typeof stResponses === 'object'
        && Object.keys(stResponses).some(k => k !== 'time_in_component'
          && typeof stResponses[k] === 'string'
          && stResponses[k].trim().length > 0)
      if (stHasComponent && stHasResponses) {
        const stStart = Date.now()
        const { scoreStrategicThinking } = await import('./strategy-fit-components/strategic-thinking-scoring.js')
        const canonicalLevelForSt = assessment?.workspace_scenario?.match_info?.level
        const stResult = await scoreStrategicThinking(anthropic, {
          role_profile: assessment?.role_profile || null,
          shell_family: assessment?.shell_family || 'office',
          roleTitle: assessment?.role_title || null,
          component: stComponent,
          responses: stResponses,
          canonical_level: Number.isFinite(canonicalLevelForSt) ? canonicalLevelForSt : null,
          role_level: assessment?.role_level || null,
        })
        if (stResult && stResult.block_id === 'strategic-thinking') {
          if (!Array.isArray(workspaceBlockScores)) workspaceBlockScores = []
          workspaceBlockScores.push(stResult)
          console.log('[score] Strategic Thinking scored in', Date.now() - stStart, 'ms | score:', stResult.score)
        } else {
          console.log('[score] Strategic Thinking scorer returned null after', Date.now() - stStart, 'ms')
        }
      }
    } catch (stErr) {
      console.error('[score] Strategic Thinking scoring failed (non-fatal):', stErr?.message)
    }
  }
  ...
  try {
    ...
    const interviewHints = interviewHintsFor(assessment?.shell_family)
...
Role: ${assessment.role_title}
Function: ${assessment?.role_profile?.function || 'unknown'}
Shell family: ${assessment?.shell_family || 'office'}
Candidate: ${candidate.name}

Vocabulary palette for this shell. Use role-native vocabulary that fits the watch-out or prediction anchor. Do NOT default to generic stakeholder / project / deliverable language when the role sits in healthcare or education:
${interviewHints}
...
Write the narrative now. Return prose only.`
  ...
  // Auto-generate the Highlight Reel share token when the assessment is
  // entitled to one. Strategy-Fit always entitles; cheaper tiers only when
  // the modular Workspace ran (which is itself only triggered by the
  // Immersive add-on). Pre-generating at scoring time means the reel is
  // shareable the moment the candidate report becomes available, rather
  // than the buyer having to click Share to mint the token first.
  // -- ALTER TABLE results ADD COLUMN IF NOT EXISTS highlight_reel_token TEXT;
  // Highlight Reel entitlement gate. Mints a token at scoring time
  // when the assessment is entitled. Three sources of entitlement:
  //   - assessment.highlight_reel_addon_purchased = true. This covers
  //     subscribers (always free on every assessment), PAYG Strategy-Fit
  //     (always included in £65), and PAYG Speed-Fit / Depth-Fit buyers
  //     who attached the £10 Highlight Reel add-on at create time. Set
  //     by /api/assessment/generate during the detector pass.
  //   - assessment_mode === 'advanced'. Defensive duplicate of the
  //     Strategy-Fit case for legacy assessments created before the
  //     highlight_reel_addon_purchased column existed.
  //   - workspace_block_scores present. Modular Workspace assessments
  //     are reel-eligible; this catches legacy modular rows where the
  //     persistence flag was not set at creation.
  const reelEntitled =
    (assessment?.highlight_reel_addon_purchased === true) ||
    (assessment?.assessment_mode === 'advanced') ||
    (Array.isArray(workspaceBlockScores) && workspaceBlockScores.length > 0)
  const highlightReelToken = reelEntitled ? uuid() : null
  ...
  const result = {
    ...
    highlight_reel_token:     highlightReelToken,
  ...
  // ── Strategy-Fit Executive Summary synthesis ────────────────────────────────
  // For mode === 'advanced' assessments, call generateExecutiveSummary
  // with the full picture of scoring results. The synthesis writes a
  // structured top-of-report panel (Executive Summary for senior-tier
  // candidates, Development Summary for junior-mid) into
  // results.executive_summary. Failure is non-blocking; the report
  // renders without the panel if the call fails.
  let executiveSummary = null
  if (assessment?.assessment_mode === 'advanced') {
    try {
      const { generateExecutiveSummary } = await import('./strategy-fit-components/executive-summary.js')
      const canonicalLevel = assessment?.workspace_scenario?.match_info?.level
      executiveSummary = await generateExecutiveSummary(anthropic, {
        role_profile: assessment?.role_profile || null,
        shell_family: assessment?.shell_family || 'office',
        roleTitle: assessment?.role_title || null,
      ...
      console.error('[score] executive summary generation failed', esErr?.message)
    }
  }
  ...
  try {
    ...
    const scoringBody = roleTitle
      ? `Overall score: ${result.overall_score}/100 for ${roleTitle}. Risk: ${result.risk_level}.`
      : `Overall score: ${result.overall_score}/100. Risk: ${result.risk_level}.`
  ...
  // ── Recruiter / HR email on every completion ────────────────────────────────
  // Two mutually-exclusive paths so the recruiter never receives two emails
  // for the same candidate:
  //   Red flag fired (score below alert_threshold OR a High-severity watch-
  //   out OR an integrity concern)  → existing detailed red-flag alert.
  //   Otherwise                     → standard scoring-complete email
  //                                   with verdict-aware subject and a
  //                                   one-line top strength / top watch-out.
  // Both paths target the assessment owner's email.
  try {
    const [
      { data: { user: ownerUser } },
      { data: ownerProfile },
    ...
    const redFlagFired = result.overall_score < threshold || hasHighWatchout || integrityConcern
    if (redFlagFired) {
      ...
      await sendScoringCompleteAlert({
        candidate,
        assessment: candidate.assessments,
        result,
        userEmail: ownerUser?.email,
      })
      console.log('[score] Scoring-complete alert sent to:', ownerUser?.email)
    }
  } catch (alertErr) {
    console.error('[score] Completion alert failed (non-fatal):', alertErr?.message)
  }
