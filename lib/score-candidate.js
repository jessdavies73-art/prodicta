...
REALITY_TIMELINE_MONTH3: [2-3 sentences. Month 2-3. Where do risks surface and where do strengths consolidate by day 90?]
...
## HIRING CONFIDENCE
Synthesise everything: overall score, pressure-fit score, watch-out severity, integrity signals, seniority calibration, trajectory, and response consistency into a single Hiring Confidence percentage. This is not the same as the overall score or pass probability. It answers: "How confident should a hiring manager be about hiring this candidate right now?"
Calibration: 85-100 strong evidence across all dimensions. 70-84 clear strengths with minor addressable gaps. 55-69 mixed signals or one significant concern. 40-54 notable gaps or integrity issues. Below 40 significant concerns across multiple dimensions.
Format exactly as one line:
HIRING_CONFIDENCE: [integer 0-100]|[plain English explanation, maximum 18 words, no em dashes, no markdown]`
  ...
  console.log('[score] PRESSURE FIT NARRATIVES section raw length:', pfNarrativesRaw.length)
  console.log('[score] PRESSURE FIT NARRATIVES raw (first 800):\n', pfNarrativesRaw.slice(0, 800))
  ...
  // Candidate type
  const candidateTypeRaw = getSection(narrativesText, 'CANDIDATE TYPE')
  ...
  // Predicted outcomes
  const predictedOutcomesRaw = getSection(narrativesText, 'PREDICTED OUTCOMES')
  const predictionsLine = predictedOutcomesRaw.split('\n').find(l => /^PREDICTIONS:/i.test(l))
  let predictions = null
  if (predictionsLine) {
    const predsStr = predictionsLine.replace(/^PREDICTIONS:\s*/i, '').trim()
    const predsObj = {}
    predsStr.split('|').forEach(part => {
  ...
  // Reality timeline
  const realityTimelineRaw = getSection(narrativesText, 'REALITY TIMELINE')
  const rtLines = realityTimelineRaw.split('\n')
  const rtWeek1Line  = rtLines.find(l => /^REALITY_TIMELINE_WEEK1:/i.test(l))
  const rtMonth1Line = rtLines.find(l => /^REALITY_TIMELINE_MONTH1:/i.test(l))
  const rtMonth3Line = rtLines.find(l => /^REALITY_TIMELINE_MONTH3:/i.test(l))
  ...
  console.log('[score] reality_timeline week1 length:', realityTimeline?.week1?.length)

  // CV comparison
  const cvComparisonRaw = getSection(narrativesText, 'CV COMPARISON')
  const cvComparisonLine = cvComparisonRaw.split('\n').find(l => /^CV_COMPARISON:/i.test(l))
  ...
  // Hiring confidence
  const hiringConfidenceRaw = getSection(narrativesText, 'HIRING CONFIDENCE')
  const hiringConfidenceLine = hiringConfidenceRaw.split('\n').find(l => /^HIRING_CONFIDENCE:/i.test(l))
  let hiringConfidence = null
  if (hiringConfidenceLine) {
    const parts = hiringConfidenceLine.replace(/^HIRING_CONFIDENCE:\s*/i, '').trim().split('|')
    const confScore = parseInt(parts[0]?.trim())
    const confExplanation = stripMd(parts[1]?.trim() || '')
    if (!isNaN(confScore) && confScore >= 0 && confScore <= 100) {
      hiringConfidence = { score: confScore, explanation: confExplanation }
    }
  }
  console.log('[score] hiring_confidence:', hiringConfidence)
