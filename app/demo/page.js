...
function DemoDashboardInner() {
  ...
  const demoRoleColor = (id = '') => {
    ...
    return DEMO_ROLE_PALETTE[h % DEMO_ROLE_PALETTE.length]
  }

  // Demo client company assignments (agency only)
  const DEMO_CLIENT_BY_CANDIDATE = {
    'demo-c1':  'Acme Retail Ltd',
    'demo-c2':  'Acme Retail Ltd',
    'demo-c3':  'Brightway Logistics',
    'demo-c4':  'Brightway Logistics',
    'demo-c5':  'Northfield Industries',
    'demo-c6':  'Acme Retail Ltd',
    'demo-c7':  'Brightway Logistics',
    'demo-c10': 'Northfield Industries',
    'demo-c11': 'Acme Retail Ltd',
  }
  ...
  const demoClientsOverview = (() => {
    ...
    for (const c of candidatesInLocation) {
      ...
      if (DEMO_PLACEMENT_HEALTH[c.id]) r.activePlacements++
      const d = c.completed_at || c.invited_at
      if (d) {
        const t = new Date(d).getTime()
        if (Number.isFinite(t) && t > r.lastActive) r.lastActive = t
  ...
  const activeClientDemo = selectedClient
          ...
          <div style={{ ...cs, marginBottom: 20, padding: isMobile ? '18px 18px' : '22px 24px' }}>
            ...
            <div style={{
              ...
              {(showAllRoles ? rolesOverview : rolesOverview.slice(0, 6)).map(r => {
                const iconColor = r.assessment_id ? demoRoleColor(r.assessment_id) : TEAL
                  ...
                  >
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                      ...
                      <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 8, padding: '7px 10px' }}>
                        ...
                        }}>{r.avgScore != null ? r.avgScore : '-'}</div>
                      </div>
                      <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 8, padding: '7px 10px' }}>
        ...
        {selectedRole && activeRole && (() => {
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: TX }}>{activeRole.role_title}</h2>
            </div>
                ...
                {stages.map(s => {
                  ...
                  return (
                    <div key={s.key} style={{
                      ...
                      <div style={{ fontSize: 10, fontWeight: 700, color: s.color, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{s.label}</div>
                      <div style={{ fontFamily: FM, fontSize: 22, fontWeight: 800, color: s.color, lineHeight: 1.1 }}>{s.value}</div>
                      ...
                      <div style={{ fontSize: 10, color: s.color, opacity: 0.75, marginTop: 2 }}>{s.short}</div>
                    </div>
                  )
                })}
              </div>
            ...
            <div style={{ ...cs, padding: 0, overflow: 'hidden' }}>
              <div style={{
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: TX }}>Candidates for this role</h3>
                  <div style={{ fontSize: 12, color: TX3, marginTop: 2 }}>
          ...
          <div style={{ ...cs, marginBottom: 20, padding: isMobile ? '18px 18px' : '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: TX3, marginTop: 2 }}>
                  {demoClientsOverview.length > 0
                    ? `${demoClientsOverview.length} client${demoClientsOverview.length !== 1 ? 's' : ''}. Click a client to drill into its candidates.`
                    : 'No client company data yet.'}
                </div>
              </div>
              {demoClientsOverview.length > 6 && (
                <button
                  type="button"
                ...
                >
                  {showAllClients ? 'Show top 6' : `View all clients (${demoClientsOverview.length})`}
                </button>
              )}
            </div>
            {demoClientsOverview.length === 0 ? (
              ...
              <div style={{
                ...
                {(showAllClients ? demoClientsOverview : demoClientsOverview.slice(0, 6)).map(cl => {
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{
                          ...
                          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <path d="M9 9h.01"/><path d="M9 12h.01"/><path d="M9 15h.01"/><path d="M9 18h.01"/>
                            <path d="M15 9h.01"/><path d="M15 12h.01"/><path d="M15 15h.01"/><path d="M15 18h.01"/>
                          </svg>
                        </div>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                        ...
                        <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 8, padding: '7px 10px' }}>
                          ...
                          }}>{cl.avgScore != null ? cl.avgScore : '-'}</div>
                        </div>
                        <div style={{ background: BG, border: `1px solid ${BD}`, borderRadius: 8, padding: '7px 10px' }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: TX3, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Recommended</div>
        ...
        {isAgency && selectedClient && activeClientDemo && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: TX }}>{activeClientDemo.client_name}</h2>
            </div>
            ...
            <div style={{
              ...
              {[
                { label: 'Total', value: activeClientDemo.total, color: TX },
                { label: 'Completed', value: activeClientDemo.completed, color: GRN },
                { label: 'Pending', value: activeClientDemo.pending, color: AMB },
                { label: 'Average score', value: activeClientDemo.avgScore != null ? activeClientDemo.avgScore : '-', color: activeClientDemo.avgScore != null ? scolor(activeClientDemo.avgScore) : TX3 },
                { label: 'Recommended', value: activeClientDemo.recommended, color: TEALD },
                { label: 'Active placements', value: activeClientDemo.activePlacements, color: TEALD },
              ].map(s => (
                <div key={s.label} style={{ textAlign: isMobile ? 'left' : 'center' }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, color: TX3, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{s.label}</div>
                  <div style={{ fontFamily: FM, fontSize: 24, fontWeight: 800, color: s.color, marginTop: 4 }}>{s.value}</div>
            ...
            <div style={{ ...cs, padding: 0, overflow: 'hidden' }}>
              <div style={{
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: TX }}>Candidates for this client</h3>
                  <div style={{ fontSize: 12, color: TX3, marginTop: 2 }}>
