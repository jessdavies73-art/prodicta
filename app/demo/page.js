  ...
  return (
    <DemoLayout active="dashboard" demoEmploymentType={demoEmploymentType}>
      ...
      <main style={{ marginLeft: isMobile ? 0 : 220, marginTop: isMobile ? 96 : 46, padding: isMobile ? '16px 16px 32px' : '32px 40px', minHeight: '100vh', background: BG, flex: 1, minWidth: 0 }}>
        ...
        {/* Candidates table + assessments panel (hidden when any filter card is active or drilled into a role) */}
        <div style={{ display: (activeFilter || selectedRole) ? 'none' : 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 20, alignItems: 'flex-start' }}>
